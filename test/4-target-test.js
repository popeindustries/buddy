...
describe('target', function () {
	...
	describe('factory', function () {
		it('should decorate a new Target instance with passed data', function () {
			var target = targetFactory({type: 'js', inputPath: path.resolve('src/some.coffee'), input: 'src/some.coffee', output: 'js', runtimeOptions: {}, fileExtensions: ['js', 'json', 'coffee', 'hbs', 'handlebars', 'dust', 'jade']});
	...
	describe('parse', function () {
		beforeEach(function () {
			this.target = targetFactory({type:'js', inputPath: path.resolve('src/js'), outputPath: path.resolve('temp'), fileExtensions:['js', 'coffee'], sources:['src'], runtimeOptions:{}});
		});
	...
	describe('process', function () {
		before(function () {
			this.target = targetFactory({type:'js', inputPath:path.resolve('src/js') , fileExtensions:[], sources:[], runtimeOptions: {}});
		});
	...
	describe('build', function () {
		beforeEach(function () {
			fileFactory.cache.flush();
			this.target = targetFactory({type:'js', inputPath: path.resolve('src/js/foo.js'), outputPath: path.resolve('temp'), fileExtensions:['js', 'coffee'], sources:['src'], runtimeOptions:{}});
		});
		afterEach(function () {
			this.target.reset();
		});
		...
		it('should execute an "afterEach" hook after each processed file is ready to write to disk', function (done) {
			...
			this.target.build(function (err, filepaths) {
				filepaths[0].should.eql(path.resolve('temp/js/foo.js'))
		...
		it('should return an error if a "before" hook returns an error', function (done) {
			this.target.before = new Function('global', 'process', 'console', 'require', 'context', 'options', 'done', 'done("oops");');
			this.target.workflow = [['load', 'compile']];
			this.target.build(function (err, filepaths) {
				should.exist(err);
				done();
		...
		it('should return an error if an "after" hook returns an error', function (done) {
			this.target.after = new Function('global', 'process', 'console', 'require', 'context', 'options', 'done', 'done("oops");');
			this.target.workflow = [['load', 'compile']];
			this.target.build(function (err, filepaths) {
				should.exist(err);
				done();
