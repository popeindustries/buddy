...
describe('config', () => {
  ...
  describe('parse', () => {
    ...
    it('should parse target "input"', () => {
      const target = config.parse([{input:'src/hey.js',output:'js'}]);
    ...
    it('should parse target array "input"', () => {
      const target = config.parse([{input:['src/hey.js', 'src/ho.js'],output:'js'}]);
    ...
    it('should parse target glob pattern "input"', () => {
      const target = config.parse([{input:'src/ma*.js',output:'js'}]);
    ...
    it('should parse target glob pattern array "input"', () => {
      const target = config.parse([{input:'src/m*.js',output:'js'}]);
    ...
    it('should not parse target "input" when not matched with "--grep" option', () => {
      const target = config.parse([{input:'src/hey.js',output:'js'}], {runtimeOptions: {grep: '*.css'}});
    ...
    it('should parse target "output_compressed"', () => {
      const target = config.parse([{input:'src/hey.js',output:'js', output_compressed:'c'}], {runtimeOptions:{compress: true},sources:[
        '.']});
      expect(target[0].output).to.eql('js');
    ...
    it('should return multiple targets when "input" and "output" are arrays of same length', () => {
      const target = config.parse([{input:['src/main.js','src/sub.js'],output:['js/main.js','js/sub.js']}]);
    ...
    it('should return a target with "appServer" set to TRUE when "server.file" is the same as "input"', () => {
      const target = config.parse([{input:'src/main.js'}], {sources:['.'],server:{file:'src/main.js'}});
    ...
    it('should return a target with "appServer" set to TRUE when "server.file" is in directory "input"', () => {
      const target = config.parse([{input:'src'}], {sources:['.'],server:{file:'src/main.js'}});

      expect(target[0]).to.have.property('appServer', true);
    });
    it('should return a target with "appServer" set to TRUE when "server.file" matches a globbed "input"', () => {
      const target = config.parse([{input:'src/*.js'}], {sources:['.'],server:{file:'src/main.js'}});

      expect(target[0]).to.have.property('appServer', true);
    });
    it('should return a target with passed "sources"', () => {
      const target = config.parse([{sources:['foo'],targets:[{input:'src/main.js', output:'js'}]}]);
    ...
    it('should return a target with an executable "before" hook function', () => {
      const func = config.parse([{input:'src/main.js',output:'js',before:'console.log(context);'}])[0].before;
    ...
    it('should return a target with an executable "before" hook function when passed a path', () => {
      const func = config.parse([{input:'src/main.js',output:'js',before:'./hooks/before.js'}])[0].before;

      expect(typeof func == 'function').to.be(true);
    });
    it('should throw an error when passed invalid "before" hook path', () => {
      try {
        const func = config.parse([{input:'src/main.js',output:'js',before:'./hook/before.js'}])[0].before;
