...
exports.load = function (configpath, fileExtensions, runtimeOptions) {
	...
	extend(exports.data.fileExtensions, fileExtensions);
	extend(exports.data.runtimeOptions, runtimeOptions);
	extend(exports.data.server, data.server);
	processWorkflows(exports.data.workflows, exports.data.runtimeOptions);
...
function parseTargets (type, sources, targets) {
	...
	for (var i = 0; i < targets.length; i++) {
		...
		target.fileExtensions = exports.data.fileExtensions[type];
		target.runtimeOptions = clone(exports.data.runtimeOptions);
		target.workflows = exports.data.workflows;
		parseInput(target);
		// Single target
...
function processWorkflow (target) {
	...
	if (target.type == 'html') {
		...
		if (target.outputPath) {
			target.workflow[0].push('parse', 'replaceReferences', 'compress')
 ...
 * @param {Object} runtimeOptions
 */
function processWorkflows (data, runtimeOptions) {
	// CSS
	data.css = [['load']];
	if (runtimeOptions.lint) data.css[0].push('lint');
	data.css[0].push('output:parse');
	data.css.push(['output:inline', 'output:compile']);
	if (runtimeOptions.compress) data.css[1].push('output:compress');

	// HTML
	data.html = [['load']];
	data.html[0].push('output:parse', 'output:replaceReferences', 'output:compress')
	data.html.push(['output:compile', 'output:inline']);

	// JS
	data.js = [['load']]
	if (runtimeOptions.lint) data.js[0].push('lint');
	// Allow for dead code removal by replacing env first
	data.js[0].push('output:replaceEnvironment', 'output:compile', 'output:parse');
	data.js[0].push('modular:output:inline', 'modular:output:replaceReferences');
	if (runtimeOptions.lazy) {
		if (runtimeOptions.compress) data.js[0].push('modular:output:compress');
		data.js[0].push('modular:output:escape');
	}
	data.js[0].push('modular:output:wrap');
	data.js.push(['modular:output:concat']);
	if (runtimeOptions.compress) data.js[1].push('modular:output:compress');
}
