...
exports.parse = function(data) {
	...
	var _parse = function(type, sources, targets) {
		...
		targets.forEach(function(target) {
			...
			target.outputpath = target.options.compress && target.output_compressed
			...
			target.workflow = [];
			if (type == 'html') {
				target.workflow.push('file:transfigure', 'file:write');
			} else if (type == 'css') {
				// Resolve dependencies even if directory for css
				target.workflow.push('file:parse', 'file:concat', 'target:filter', 'file:transfigure');
				if (target.options.compress) target.workflow.push('file:compress');
				target.workflow.push('file:write');
			} else {
				if (target.isDir) {
					target.workflow.push('file:transfigure');
					if (target.modular) target.workflow.push('file:wrap');
					if (target.options.compress) target.workflow.push('file:compress');
					target.workflow.push('file:write');
				} else {
					target.workflow.push('file:transfigure');
					if (target.modular) {
						...
						if (target.options.lazy) target.workflow.push('file:escape');
						if (target.options.lazy && target.options.compress) target.workflow.push('file:compress');
						target.workflow.push('file:wrap', 'target:resolve', 'file:concat');
					}
					if (target.options.compress) target.workflow.push('file:compress');
					target.workflow.push('file:write');
				}
