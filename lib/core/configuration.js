...
exports.parse = function(data, options) {
	...
	var _parse = function(type, sources, targets) {
		...
		targets.forEach(function(target) {
			...
			target.outputPath = target.options.filepath = options.compress && target.output_compressed
			...
			target.workflow = [];
			if (type == 'html') {
				target.workflow.push('transfigure', 'target:output');
			} else if (type == 'css') {
				...
				target.workflow.push('target:output');
			} else {
				if (target.isDir) {
					target.workflow.push('transfigure');
