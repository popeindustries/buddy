...
var DEFAULT_JS = 'buddy.js'
	...
	, DEFAULT_SOURCES = {
			...
			html: ['.']}
	, FILE_EXTENSIONS = {
			js: ['js', 'json', 'coffee', 'hbs', 'handlebars', 'dust', 'jade', 'nunjucks', 'nunjs', 'jsx'],
			css: ['css', 'styl', 'less'],
...
exports.parse = function (data) {
	...
	var _parseTargets = function (type, sources, targets) {
		...
		targets.forEach(function (target) {
			...
			target.isAppServer = !target.modular && exports.data.server && (exports.data.server.file == target.input);

			// Output is optional
			if (target.output) {
				// Handle specified compressed output path
				var outputPath = (target.runtimeOptions.compress && target.output_compressed)
					? target.output_compressed
					: target.output;
				// Handle relative paths (relative to input)
				target.outputPath = (outputPath.charAt(0) == '.')
					? path.resolve(target.isDir ? target.inputPath : path.dirname(target.inputPath), outputPath)
