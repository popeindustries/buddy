...
exports.load = function(url, options, fn) {
	...
	exports.locate(url, function(err, url) {
		...
		try {
			...
			exports.data.options = object.extend({}, DEFAULT_OPTIONS, options);
...
exports.parse = function(data) {
	...
	var _parse = function(type, targets) {
		...
		targets.forEach(function(target) {
			...
			target.outputpathCompressed = target.output_compressed
				...
				: target.outputpath;
			target.options = object.clone(exports.data.options);
			...
			if (target.targets) {
				...
				target.targets = _parse(type, target.targets);
			}
			// Store
			valid.push(target);
		});
	...
	for (var type in data) {
		...
		if (b
			...
			&& b.targets.length) {
				...
				// Store targets
				build.targets = build.targets.concat(_parse(type, b.targets));
		} else {
	...
	return build.sources.length && build.targets.length ? build : null;
};
