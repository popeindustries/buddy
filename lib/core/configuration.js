...
exports.parse = function(data) {
	...
	var _parse = function(type, sources, targets) {
		...
		targets.forEach(function(target) {
			...
			target.workflow = [];
			if (type == 'html') {
				target.workflow.push('compile', 'target:write');
			} else if (type == 'css') {
				...
				if (target.isDir) {
					...
					if (target.modular) {
						...
						if (target.runtimeOptions.lazy) target.workflow.push('escape');
						if (target.runtimeOptions.lazy && target.runtimeOptions.compress) target.workflow.push('compress');
