...
var DEFAULT_JS = 'buddy.js'
	...
	, DEFAULT_SOURCES = {
			...
			html: ['.']}
	, FILE_EXTENSIONS = {
			js: ['js', 'coffee', 'ls', 'hbs', 'handlebars', 'dust'],
			css: ['css', 'styl', 'less'],
			html: ['html', 'jade', 'twig']};
...
exports.load = function(url, runtimeOptions, fn) {
	...
	exports.locate(url, function(err, url) {
		...
		try {
			...
			// Backwards compatibile refactor of test -> script
			if (runtimeOptions.test != null) exports.data.runtimeOptions.script = runtimeOptions.test;
			if (data.settings && data.settings.test != null) exports.data.settings.script = data.settings.test;
			// Validate
...
function processWorkflow (type, target) {
	...
	if (type == 'html') {
		...
		if (target.isDir) {
			...
			if (target.modular) {
				target.workflow.push('file:parse');
				if (target.runtimeOptions.deploy) target.workflow.push('file:replace');
...
function defineHook (hook) {
	...
	return new Function('context', 'options', 'callback', hook);
}