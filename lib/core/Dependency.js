...
var Dependency, RE_GITHUB_PROJECT, RE_GITHUB_URL, RE_INDEX, RE_PACKAGE_NOT_FOUND, RE_VALID_VERSION, async, bower, cp, existsSync, fs, fsutils, http, mkdir, mv, path, request, rm, semver, unzip, _ref,
  ...
  Dependency.prototype.move = function(fn) {
    ...
    if (!this.keep) {
      ...
      return async.forEachSeries(this.resources, (function(resource, cb) {
        return fsutils[_this.local ? 'cp' : 'mv'](resource, _this.destination, function(err, filepath) {
          if (err) {
            return cb(err);
          }
          idx++;
