...
File.prototype.compile = function () {
	...
	if (!this.compiledContent) {
		...
		if (this.type == 'html') {
			...
			// 				if (dependency = opts.fileCache.getFile(dependency.filepath)) {
			// 					// Recursive
		...
		return commands['compile'](this.filepath, this.content, clone(this.options))
			.then(function (content) {
				// Store
	...
	return commands['parse'](this.filepath, this.type, this.content)
		.then(function (dependencies) {
			return Promise.all(dependencies.map(function (dependency) {
				if (filepath = resolve(self.filepath, dependency, self.options)) {
					file = factory(filepath, self.options);
					// Store
					if (!~self.dependencies.indexOf(file)) {
...
File.prototype.replace = function () {
	...
	return commands['replace'](this.content)
		.then(function (content) {
...
File.prototype.wrap = function () {
	...
	return commands['wrap'](this.id, this.content, this.runtimeOptions.lazy)
		.then(function (content) {
			self.content = content;
