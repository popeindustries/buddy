...
File.prototype.run = function (workflow) {
	...
	if (workflow && !this.isRunning) {
		...
		return Promise.reduce(workflow, function(noArg, task) {
...
File.prototype.compile = function () {
	...
	if (!this.compiledContent && this.isWriteable) {
		var self = this
			, options = {}
			, dataUrl;
		...
		if (this.type == 'html') {
			...
			options.includes = this.dependencies.concat(flatten(this.dependencies, 'dependencies'));
			...
			if (dependencies) {
				return Promise.map(dependencies, function (dependency) {
					...
					if (filepath = resolve(self.filepath, dependency.filepath, self.options)) {
						...
						return dependency.instance.isRunning
							? Promise.resolve()
