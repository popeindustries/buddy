...
File.prototype.run = function* (workflow) {
	...
	if (workflow && !this.isRunning) {
		...
		for (var i = 0, n = workflow.length; i < n; i++) {
			cmd = workflow[i];
			// Parse needs special handling
			if (cmd == 'parse') {
				dependencies = yield this.parse();
				// Run nested dependencies before returning
				if (dependencies && dependencies.length) {
					dependencies = yield dependencies.map(function (dependency) {
						return dependency.run(workflow);
					});
					files = flatten(files.concat(dependencies));
				}
			} else {
				// All other commands
				yield this[cmd]();
	...
	if (dependencies) {
		dependencies.forEach(function (dependency) {
			...
			if (filepath && instance) {
				...
				// console.log(this.id, instance.id, instance.isLocked)
				if (!instance.isLocked) {
						// Store if not already stored
					if (!~this.dependencies.indexOf(instance)
							// ...and if not circular dependency
							&& !~this.dependants.indexOf(instance)) {
								instance.dependants.push(this);
...
File.prototype.concat = function* () {
	...
	if (this.getIsWriteable()) {
		this.content = helpers['concat'](this.type, this.content, this.getDependencies(true));
...
File.prototype.write = function* (filepath, options) {
	if (filepath && this.getIsWriteable()) {
		options = options || {};
