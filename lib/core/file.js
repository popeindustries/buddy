...
module.exports = function(filepath, options, fn) {
	...
	if (id = identify(filepath, options)) {
		var instance = new File(id, filepath, options);
...
File.prototype.transfigure = function(fn) {
	transfigure(this.filepath, function(err, transContent, content) {
		...
		this._transContent = transContent;
		fn();
	}.bind(this));
};
...
File.prototype.escape = function(fn) {
	...
	this._transContent = escape(this._transContent);
	fn();
};

File.prototype.compress = function(fn) {
	compress(this.type, this._transContent, function(err, transContent) {
		if (err) return fn(err);
...
File.prototype.wrap = function(fn) {
	this._transContent = wrap(this.type, this.id, this._transContent, false);
	fn();
}

File.prototype.wrapLazy = function(fn) {
	this._transContent = wrap(this.type, this.id, this._transContent, true);
	fn();
}

File.prototype.resolve = function(fn) {
	this.dependencies = parse(this.type, this._transContent);
	fn();
};
