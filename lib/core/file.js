...
File.prototype.compile = function(options, fn) {
	if (!this.compiledContent) {
		var self = this
			, opts = extend(clone(this.runtimeOptions), clone(options))
			, dataUrl;

		// Expose properties for compilers
		opts.id = this.id;
		opts.type = this.type;
		opts.dependencies = this.dependencies.map(function (dependency) {
			return opts.fileCache.getFile(dependency.filepath);
		});
		// Check for data file of same name in same directory
		if (this.type == 'html'
			&& fs.existsSync((dataUrl = path.resolve(path.dirname(this.filepath), this.name.replace(this.extension, 'json'))))) {
				opts.data = require(dataUrl);
		}

		commands['compile'](this.filepath, this.content, opts, function(err, content) {
...
File.prototype.parse = function(options, fn) {
	...
	if (!~this.process.indexOf('parse')) {
		...
		commands['parse'](this, extend(clone(this.runtimeOptions), clone(options)), function(err, file) {
			...
			if (err) {
				err.forEach(function(filepath) {
					warn("failed inlining json " + (strong(filepath)), 4);
				});
			}
...
File.prototype.write = function(options, fn) {
	...
	if (options.boilerplate) {
		this.content = fs.readFileSync(path.resolve(__filename, '../../../node_modules/simple-browser-require/require.js'), 'utf8')
			+ '\n'
			+ this.content;
	}
	// Add bootstrap call
