...
var File, RE_IGNORE_FILE, Source, existsSync, indir, path, readdir, _ref,
...
_ref = require('../utils/fs'), indir = _ref.indir, readdir = _ref.readdir, existsSync = _ref.existsSync;
...
module.exports = Source = (function() {
  ...
  Source.prototype.parse = function(fn) {
    ...
    return this.locations.forEach(function(location) {
      outstanding++;
      return readdir(location, RE_IGNORE_FILE, function(err, files) {
        outstanding--;
        if (err) {
          return fn(err);
        }
  ...
  Source.prototype.add = function(filepath) {
    ...
    if (!this.byPath[filepath] && (file = this._fileFactory(filepath))) {
      this.length++;
  ...
  Source.prototype._validFileType = function(filepath) {
    var compiler, extension, name, _ref1;
    extension = path.extname(filepath).slice(1);
    if (extension === this.type) {
      return true;
    }
    _ref1 = this.processors.compilers;
    for (name in _ref1) {
      compiler = _ref1[name];
  ...
  Source.prototype._fileFactory = function(filepath) {
    var base, location, _i, _len, _ref1;
    if (this._validFileType(filepath)) {
      ...
      if (base) {
        return new File(this.type, filepath, base, this.processors);
      }
    } else {
      return null;
    }
