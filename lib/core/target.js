...
Target.prototype.build = function(fn) {
	...
	if (this.before) {
		...
		this.executeHook('before', this, function(err) {
...
Target.prototype.write = function(files, fn) {
	...
	async.each(files, function(file, cb) {
		...
		if (self.afterEach) {
			...
			self.executeHook('afterEach', file, function(err) {
				if (err) return cb(err);
				file.write(opts, writeHandler(cb, file.outputPath));
			});
		} else {
			file.write(opts, writeHandler(cb, file.outputPath));
		}
 ...
 */
Target.prototype.executeHook = function(hook, context, callback) {
	// Make global objects available to the function
	this[hook].call(context, global, process, console, require, context, this.runtimeOptions, callback);
};
