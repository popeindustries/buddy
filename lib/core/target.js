...
/**
 * Filter sourceFiles to remove dependencies
 * @param {Array} files
 * @param {Function} fn
 */
Target.prototype.filter = function(files, fn) {
	this.outputFiles = this.outputFiles.filter(function(file) {
 ...
 * @param {Function} fn
 */
Target.prototype.output = function(files, fn) {
	var self = this;
	async.each(files, function(file, cb) {
		file.write(self.options, function(err, filepath) {
			if (err) return cb(err);
			// Store
			self.outputPaths.push(file.filepath);
			cb();
		});
	}, fn);
};
