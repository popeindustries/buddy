...
Target.prototype.build = function () {
	...
	if (this.before) {
		...
		sequence.then(function () {
			return self.executeHook('before', this);
		});
	}
	...
	if (this.after) {
		...
		sequence.then(function (filepaths) {
			return self.executeHook('after', this);
		}).return(filepaths);
	}
	// Return output paths on completion
	return sequence.then(function (filepaths) {
		return filepaths;
	});
};
	...
	return Promise.all(files.map(function (file) {
		if (file.isWriteable) {
			...
			if (self.afterEach) {
				...
				sequence.then(function () {
					return self.executeHook('afterEach', this);
				});
			}
			// Write file
...
Target.prototype.executeHook = function (hook, context) {
	...
	return Promise.nodeify(this[hook].call(context, global, process, console, require, context, this.runtimeOptions));
};
