...
module.exports = function(type, options, fn) {
	...
	fs.stat(inputpath, function(err, stats) {
		...
		// Override options
		if (type == 'html') {
			options.modular = options.concat = options.compress = options.ling = false;
		} else {
...
Target.prototype._parse = function(fn) {
	...
	if (this.isDir) {
		// Grab files
		readdir(this.input, null, ignored, function(err, files) {
...
Target.prototype._compile = function(content, filepath, compiler, fn) {
	if (this.options.compile && compiler) {
		compiler.compile(content, filepath, function(err, content) {
			if (err) return fn(err);
