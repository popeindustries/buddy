...
Target.prototype.build = function () {
	...
	return self.executeHook('before', self)
		.then(function () {
			...
			if (self.hasChildren) self.lock(self.referencedFiles);
			return filepaths;
		}).then(function (filepaths) {
			// Execute 'after' hook
...
Target.prototype.process = function (files, workflow) {
	...
	if (files.length) {
		...
		return Promise.reduce(workflow, function (files, tasks) {
			// Execute tasks in parallel
			return Promise.map(files, function (file) {
				return file.run(tasks);
			}).then(function (files) {
				// Prepare array of files for next group
