...
const { regexpEscape, replace, truncate } = require('./utils/string');
...
module.exports = class File {
  ...
  inlineDependencyReferences () {
    function inline (string, references) {
      let context;

      references.forEach((reference) => {
        // Inline nested (duplicates are allowed)
        context = reference.file.dependencyReferences.length
          ? inline(reference.file.string, reference.file.dependencyReferences)
          : reference.file.string.toString();

        if (reference.context) replace(string, new RegExp(regexpEscape(reference.context), 'g'), context);
      });
    }

    // TODO: remove comments?
    inline(this.string, this.dependencyReferences);
  }
  ...
  load (buildOptions, fn) {
    if (!this.content) {
      // TODO: replace line endings
      this.content = readFile(this.filepath, this.encoding);
      this.hash = md5(this.content);

      debug(`load: ${strong(this.relpath)}`, 4);
    }

    if (this.encoding == 'utf8') this.string = new MagicString(this.content);
  ...
  write (buildOptions, fn) {
    this.run('writeable', buildOptions, (err) => {
      ...
      // if (this.map) {
      //   const mappath = this.writepath.replace(path.extname(this.writepath), '.map');
      //   const map = this.map.toStringWithSourceMap({ file: path.basename(this.writepath) }).map;
      ...
      writeFile(this.writepath, content, this.encoding);
