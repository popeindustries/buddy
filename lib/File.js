...
module.exports = class File {
  ...
  replaceContent (string, index, content) {
    ...
    string.forEach((args, idx) => {
      this._replaceContent(args[0], args[1], location[idx].line, args[2]);
    });
  }
   ...
   * @param {String|File} content
   */
  _replaceContent (string, index, line, content) {
    let contentLines = 0;

    if ('string' == typeof content) {
      contentLines = content.split('\n').length - 1;
      if (contentLines) sourceMap.insert(this.map, null, contentLines, line);
    } else {
