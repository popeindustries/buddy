...
module.exports = class File {
  ...
  /**
   * Append 'chunk' with optional 'offset'
   * @param {String|File} chunk
   * @param {Number} offset
   */
  append (chunk, offset = 0) {
    offset = offset
    ...
    if ('string' == typeof chunk) {
      ...
      chunk = offset + file.content;
      sourceMap.append(this.map, file.map, this.totalLines);
    }
    this.content += chunk;
    this.totalLines += chunk.split('\n').length + 1;
  }

  prepend (chunk, offset = 0) {
