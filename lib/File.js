...
const { getLocationFromIndex, regexpEscape, truncate } = require('./utils/string');
...
module.exports = class File {
  ...
  /**
   * Replace 'string' at 'location' with 'content'
   * @param {String|File} content
   * @param {Number} index
   * @param {String} string
   */
  replaceContent (content, index, string) {
    const location = getLocationFromIndex(this.content, index);
    let contentLines = 0;

    if ('string' == typeof content) {
      contentLines = content.split('\n').length - 1;
      if (contentLines) sourceMap.insert(this.map, null, contentLines, location.line);
    } else {
      contentLines = content.totalLines;
      sourceMap.insert(this.map, content.map, contentLines, location.line);
      string = content.content;
    }

    console.log(this.content.substring(0, index), this.content.substring(index + string.length))
  ...
  load (buildOptions, fn) {
    ...
    if (this.encoding == 'utf8') this.setMap();
