...
module.exports = class File {
   ...
   * @returns {Object}
   */
  parseWorkflows (buildOptions, workflows = this.workflows) {
    let parsedWorkflows = {};

    for (const type in workflows) {
      parsedWorkflows[type] = workflows[type].reduce((tasks, task) => {
        if (~task.indexOf(':')) {
  ...
  runWorkflow (workflowType, buildOptions, fn) {
    const workflow = this.parseWorkflows(buildOptions)[workflowType];
