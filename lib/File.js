...
module.exports = class File {
  ...
  getAllDependencies () {
    ...
    function walkDependencies (dependencies) {
      for (const dependency of dependencies) {
        if (!dependency.isCircularDependency) walkDependencies(dependency.dependencies);
        if (!dependency.dependencyReferences.size) {
          allDependencies.add(dependency);
        } else if (!toSort.includes(dependency)) {
          toSort.push(dependency);
        }
      }
    }

    function walkDependencyReferences (references) {
      for (const reference of references) {
        const { file } = reference;

        if (file && !reference.isCircular) {
          if (!allDependencies.has(file)) {
            walkDependencyReferences(file.dependencyReferences);
            allDependencies.add(file);
          }
        }
      }
    }

    walkDependencies(this.dependencies);
    sortBy(toSort, 'depth')
      .forEach((file) => {
        if (!allDependencies.has(file)) {
          walkDependencyReferences(file.dependencyReferences);
  ...
  addDependencies (dependencies, buildOptions) {
    ...
    this.dependencyReferences = new Set(dependencies);
    this.dependencyReferences.forEach((dependency) => {
      ...
      if (filepath === '') {
        this.dependencyReferences.delete(dependency);
      ...
      if (file.isDependency) {
        ...
        for (const dependency of file.dependencyReferences) {
  ...
  reset (hard) {
    ...
    this.dependencyReferences.clear();
    this.depth = 0;
    this.helpers = [];
    this.ran = 0;
    if (!hard) {
