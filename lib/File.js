...
module.exports = class File {
  ...
  inlineDependencyReferences () {
    function inline (content, references) {
      ...
      references.forEach((reference) => {
        ...
        if (reference.context) content = content.replace(new RegExp(regexpEscape(reference.context), 'gm'), inlineContent);
      });
      return content;
    }
  ...
  load (buildOptions, fn) {
    if (!this.fileContent) {
      ...
      this.fileContent = readFile(this.filepath, this.encoding);
      this.hash = md5(this.fileContent);

      debug(`load: ${strong(this.relpath)}`, 4);
    }

    this.content = (this.encoding == 'utf8')
      ? new MagicString(this.fileContent)
      : this.fileContent;
  ...
  prepareForWrite (filepath, buildOptions) {
    ...
    this.writeHash = md5([this.content.toString(), ...this.getAllDependencies().map((file) => file.content.toString())].join('\n'));
