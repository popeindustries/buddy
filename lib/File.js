...
module.exports = class File {
  ...
  prepareForWrite (filepath, buildOptions) {
    this.hash = md5([this.content, ...this.getAllDependencies().map((file) => file.content)].join('\n'));

    // Generate unique path
    if (isUniqueFilepath(filepath)) {
      // Disable during watch otherwise css reloading won't work
      filepath = generateUniqueFilepath(filepath, !this.options.runtimeOptions.watch ? this.hash : false);
    }
