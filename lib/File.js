...
const { ensureAsync, series, parallel } = require('async');
...
module.exports = class File {
   ...
   * @returns {Array}
   */
  getWorkflows (buildOptions) {
    let workflows = {};

    for (const type in this.workflows) {
      workflows[type] = this.workflows[type].reduce((tasks, task) => {
        if (~task.indexOf(':')) {
   ...
   */
  run (buildOptions, fn) {
    const workflows = ['default', 'extra'];

    series(workflows.map((workflowType) => callable(this, 'runWorkflow', workflowType, buildOptions)),
      (err) => {
  ...
  runWorkflow (workflowType, buildOptions, fn) {
    const workflow = this.getWorkflows(buildOptions)[workflowType];
