...
const { indent, regexpEscape, truncate } = require('./utils/string');
...
module.exports = class File {
  ...
  getAllDependencies () {
    ...
    function add (dependencies) {
      for (const dependency of dependencies) {
        if (!dependency.isCircularDependency) add(dependency.dependencies);
    ...
    return sortBy(allDependencies, 'depth').reverse();
  }
  ...
  addDependencies (dependencies, buildOptions) {
    ...
    for (const dependency of this.dependencyReferences) {
      ...
      if (file.isDependency) {
        ...
        // Only reparent if moving to greater depth
        if (file.parent.depth < this.depth) {
          file.reparent(this);
        }
      } else {
  ...
  /**
   * Establish relationship with new 'parent'
   * @param {File} parent
   */
  reparent (parent) {
    this.parent.dependencies.delete(this);
    this.parent = parent;
    this.depth = parent.depth + 1;
    parent.dependencies.add(this);
    for (const dependency of this.dependencyReferences) {
      if (dependency.file
        && dependency.file.depth < this.depth + 1
        && !dependency.isIgnored
        && !dependency.isCircular) {
          dependency.file.reparent(this);
      }
    }
  }
