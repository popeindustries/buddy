...
module.exports = class File {
  ...
  getAllDependencies () {
    ...
    function add (dependencies, depth) {
      dependencies.forEach((dependency) => {
        if (dependency.depth < depth) dependency.depth = depth;
        if (!allDependencies.includes(dependency)) {
          allDependencies.push(dependency);
          add(dependency.dependencies, depth + 1);
        }
      });
    }

    add(this.dependencies, 1);

    // Depth first sort
    return sortBy(allDependencies, 'depth');
  }
    ...
    dependencies.forEach((dependency) => {
      ...
      if (file.isLocked || ignoredFiles.includes(filepath)) {
        ...
        if (file.isDependency) {
          ...
          if (file.dependencies.includes(this)) {
            // Flag in parent
            file.dependencyReferences.some((dependency) => {
          ...
          // Filter own dependencies
          file.isDependency = true;
          this.dependenciesOwn.push(file);
        }
  ...
  runForDependencies (type, buildOptions, dependencies, fn) {
    dependencies = dependencies || this.dependenciesOwn;
  ...
  runWorkflowForDependencies (type, index, buildOptions, fn) {
    parallel(this.dependenciesOwn.map((dependency) => {
