...
module.exports = class File {
  ...
  addDependencies(dependencies, buildOptions) {
    ...
    dependencies.forEach(dependency => {
      ...
      if (!filepath || !exists(filepath)) filepath = resolve(this.filepath, dependency.id, resolveOptions);
  ...
  run(type, buildOptions, fn) {
    ...
    if (this.isInline && type == 'standard') type = 'inlineable';
  ...
  runForDependencies(type, dependencies, buildOptions, fn) {
    ...
    if (!dependencies.length) return fn();
   ...
   * @returns {String}
   */
  hashContent(content, forWrite = false) {
    return forWrite
      ? md5([content, ...this.getAllDependencies().map(file => file.content)].join(''))
      : md5(content);
  }
  ...
  load(buildOptions, fn) {
    if (!this.fileContent) {
      ...
      this.hash = this.hashContent(this.fileContent, false);
  ...
  prepareForWrite(filepath, buildOptions) {
    ...
    this.writeHash = this.hashContent(this.content, true);
    console.log(this.id, this.writeHash)
