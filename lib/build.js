...
class Build {
  ...
  constructor (props) {
    ...
    if (this.inputpaths.length > 1) {
      ...
      if (this.inputString.length > 3) {
        this.inputString = `${this.inputString.slice(0, 3).join(', ')} ...and ${(this.inputString.length - 3)} others`;
      } else {
  ...
  processFiles (files, fn) {
    ...
    parallel(files.map((file) => callable(file, 'run', runOptions)),
      (err, results) => {
        if (err) return fn(err);
        this.referencedFiles = distinct(files.push(...flatten(results)));
   ...
   * @param {Array} files
   * @param {Function} fn(err, files, filepaths)
   */
  writeFiles (files, fn) {
    const written = files
