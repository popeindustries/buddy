...
class Build {
   ...
   */
  hasFile (file) {
    if (this.referencedFiles.some((refFile) => refFile.filepath == file.filepath)) return true;
...
function generatePathString (paths) {
  ...
  if (paths.length > 1) {
    pathString = paths.map((pathItem) => pathname(pathItem));
    // Trim long lists
