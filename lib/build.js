...
class Build {
  ...
  init (fn) {
    ...
    if (this.isGeneratedBuild) {
      print(`${chalk.grey(this.printPrefix + '>')} ${type} ${strong(this.outputString)}`, 1);
    } else {
      print(`${chalk.grey(this.printPrefix + '>')} ${type} ${strong(this.inputString)} ${this.outputString ? 'to ' + strong(this.outputString) : ''}`, 1);
    }
  ...
  processGeneratedBuild (fn) {
    ...
    function getFiles (builds) {
      return builds.reduce((files, build) => {
        files.push(...build.inputFiles.reduce((files, file) => {
          // Use dependencyReferences to include those missing from dependencies due to locking etc.
          return files.concat(...file.dependencyReferences
    ...
    if (RE_GENERATED_SHARED.test(this.generatedInputPattern)) {
      ...
      matchingFiles = unique(getFiles(this.builds))
        .filter((file) => match(file.filepath, this.generatedInputPattern, { matchBase: true, nocase: true }));
    }
  ...
  printWriteProgress (results, fn) {
    ...
    const prints = results
      .reverse()
      .map((result) => {
        return callable(printResult, null, result, this.runtimeOptions.deploy, this.runtimeOptions.compress, result.printPrefix);
      });
...
function generatePathString (paths) {
  ...
  if (paths.length > 1) {
    ...
    if (pathString.length > maxInputStringLength) {
      ...
      pathString = `${pathString.slice(0, maxInputStringLength).join(', ')} ...and ${remainder} other${remainder > 1 ? 's' : ''}`;
    } else {
...
function printResult (result, isDeploy, isCompressed, prefix, fn) {
  ...
  if ((result.type == 'js' || result.type == 'css') && isDeploy) {
    zlib.gzip(result.content, (err, buffer) => {
      ...
      print(chalk.green(`${chalk.grey('<' + prefix)} built and compressed ${strong(relpath)}`), 1);
      print(`[compressed size: ${chalk[over ? 'red' : 'green'](prettyBytes(bytes))}]`, 3 + prefix.length);
      print(`[gzipped size: ${chalk[overZipped ? 'red' : 'green'](prettyBytes(buffer.length))}]`, 3 + prefix.length);
      if (over || overZipped) {
        warn(`the output file exceeds the recommended ${strong(prettyBytes(recommendedFileSizeLimit))} size`, 3 + prefix.length);
        print('Consider splitting into smaller bundles to help improve browser startup execution time', 3 + prefix.length);
      }
      fn();
    });
  } else {
    print(chalk.green(`${chalk.grey('<' + prefix)} built${isCompressed ? ' and compressed' : ''} ${strong(relpath)}`), 1);
    fn();
  }
}