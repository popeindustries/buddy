...
class Build {
  ...
  processGeneratedBuild (fn) {
    ...
    if (RE_GENERATED_SHARED.test(this.generatedInputPattern)) {
      ...
      const sharedFiles = (function getFiles (builds) {
        ...
        .reduce((sharedFiles, file) => {
          if (!(file.id in seen)) {
            seen[file.id] = 1;
          } else if (!sharedFiles.includes(file)) {
            sharedFiles.push(file);
          }
          return sharedFiles;
        }, []);
      // Generate dummy dependency references
      const sharedDependencies = sharedFiles.map((file) => {
        file.isDependency = false;
        return { filepath: file.filepath };
      });

      dummyFile.allDependencies = dummyFile.allDependencyReferences = null;
      dummyFile.addDependencies(sharedDependencies, this.processFilesOptions);
