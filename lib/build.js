...
class Build {
  ...
  run (fn) {
    ...
    waterfall([
      ...
      // Build child targets
      callable(this, 'runChildren'/* , referencedFiles */),
      // Write files
      callable(this, 'writeFiles'/* , referencedFiles, childResults */),
      // Execute 'after' hook
      callable(this, 'executeHook', 'after', [this]/* , results */),
      // Reset
  ...
  writeFiles (files, childResults, fn) {
    ...
    parallel(writeable, (err, results) => {
      ...
      env('OUTPUT', results.map((item) => item.filepath), this.id);
      env('OUTPUT_HASH', results.map((item) => item.hash), this.id);
      env('OUTPUT_DATE', results.map((item) => item.date), this.id);
