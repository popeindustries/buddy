...
class Build {
  ...
  hasFile (filepath) {
    ...
    if (this.hasChildren) return this.builds.some((build) => build.hasFile(filepath));
  ...
  runProcessForChildren (files, fn) {
    if (!this.hasChildren) return fn(null);
   ...
   */
  writeFiles (files, fn) {
    const writeable = files.map((file) => callable(file, 'write', file.writepath, this.processFilesOptions));
