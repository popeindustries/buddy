...
const fs = require('fs');
const intersection = require('lodash/intersectionWith');
...
class Build {
  ...
  runProcess (fn) {
    waterfall([
      ...
      // Process intersection files
      callable(this, 'processIntersectionFiles')
    ], fn);
  }
  ...
  /**
   * Process intersection of child files
   * @param {Function} fn(err)
   * @returns {null}
   */
  processIntersectionFiles (fn) {
    if (!this.isIntersectionBuild) return fn();

    const dummyFile = this.inputFiles[0];
    // Determine intersection based on child build input files, not child build reference files
    // Allows pruning common files between batched files in same build
    const commonFiles = intersection(this.builds
      .reduce((files, build) => files.concat(build.inputFiles), [])
      .map((file) => file.getAllDependencies()), (a, b) => a.id == b.id);
    const commonDependencies = commonFiles[0].map((file) => {
      file.isDependency = false;
      return { filepath: file.filepath };
    });

    dummyFile.allDependencies = dummyFile.allDependencyReferences = null;
    dummyFile.addDependencies(commonDependencies, this.processFilesOptions);
...
function printResult (result, isDeploy, isCompressed, level, fn) {
  ...
  if ((result.type == 'js' || result.type == 'css') && isDeploy) {
    zlib.gzip(result.content, (err, buffer) => {
      ...
      if (over || overZipped) {
        warn(`the output file exceeds the recommended ${strong(prettyBytes(recommendedFileSizeLimit))} size`, 3 + level);
