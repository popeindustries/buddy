...
class Build {
  ...
  init (fn) {
    ...
    if (this.isGeneratedBuild) {
      ...
      print(`${this.printPrefix} ${type} ${strong(this.inputString)} ${this.outputString ? 'to ' + strong(this.outputString) : ''}`, 1);
    }
...
function printResult (result, isDeploy, isCompressed, prefix, fn) {
  ...
  if ((result.type == 'js' || result.type == 'css') && isDeploy) {
    zlib.gzip(result.content, (err, buffer) => {
      ...
      if (over || overZipped) {
	warn(`the output file exceeds the recommended ${strong(prettyBytes(recommendedFileSizeLimit))} size`, 2 + prefix.length);
	print('Consider splitting into smaller bundles to help improve browser startup execution time', 2 + prefix.length);
      }
      fn();
    });
  } else {
