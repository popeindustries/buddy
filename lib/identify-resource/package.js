...
exports.get = function (filepath, options) {
  ...
  const pkgname = exports.resolveName(pkgpath);
  ...
  details.id = details.name + (!details.isRoot && details.version ? config.VERSION_DELIMITER + details.version : '');
...
exports.resolveId = function (details, filepath) {
  ...
  if ('string' == typeof filepath) {
    ...
    const version = (!details.isRoot && details.version) ? config.VERSION_DELIMITER + details.version : '';
...
function parseNodeModules (pkgpath) {
  ...
  let dir = pkgpath;
  let dirs = [];
  let details, parent, nodeModulespath;
