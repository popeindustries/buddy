...
var Builder, CSS, HTML, JS, Source, async, colour, configuration, debug, dependencies, error, existsSync, filelog, fs, notify, path, print, processors, readdir, rm, start, strong, target, warn, _ref, _ref1,
...
module.exports = Builder = (function() {
  ...
  function Builder() {
    this._buildTarget = __bind(this._buildTarget, this);
  ...
  Builder.prototype.build = function(configpath, compress, lint, verbose, fn) {
    var _this = this;
          ...
          if (_this._validBuildType(build)) {
            ...
            return _this.sources[type].parse(function(err) {
              ...
              return _this._parseTargets(type, build.targets, function(err, instances) {
                ...
                debug('BUILD', 1);
                return async.forEachSeries(_this.targets[type], (function(target, cb2) {
                  return _this._buildTarget(target, compress, lint, function(err) {
                    if (err) {
                      return cb2(err);
                    }
                    return cb2();
  ...
  Builder.prototype.watch = function(configpath, compress, verbose) {
    var _this = this;
    this.compress = compress;
    return this._initialize(configpath, verbose, function(err) {
      if (err) {
      ...
      _this.build(configpath, _this.compress, false, verbose);
      _this.watching = true;
      return [JS, CSS].forEach(function(type) {
        if (_this[type + 'Sources'].count) {
          print("watching [" + (strong(_this.config.build[type].sources.join(', '))) + "]...", 0);
  ...
  Builder.prototype.deploy = function(configpath, verbose) {
    return this.build(configpath, true, false, verbose);
  };
  ...
  Builder.prototype._buildTarget = function(target, compress, lint, fn) {
    var _this = this;
    return target.build(compress, lint, function(err, files) {
      files && filelog.add(files);
      if (err) {
        return fn(err);
      }
