...
class Builder {
  ...
  init (configpath, options) {
    ...
    if (!this.initialized) {
      // Load compilers
      const fileExtensions = transfigure.load();
    ...
    if (!this.building) {
      ...
      this.buildTargets(targets, (err, filepaths) => {
        ...
        if (err) {
          ...
          if (serverfarm) {
            const filepath = filepaths.length ? filepaths[0] : 'foo.js';
