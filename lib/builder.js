	...
	this._initialize(configpath, options, function(err) {
		...
		if (self.config.dependencies) {
			...
			try {
				...
				error('buddy-dependencies add-on missing. Install \'buddy-dependencies\' with npm', 2);
			}
		} else {
	...
	async.eachSeries(this.targets, (function(target, cb) {
		if (target.type == type || type == '*') {
			target.build(function(err, files) {
				...
				// Reload
				if (serverfarm) serverfarm.refresh(path.basename(files[0]));
				cb();
			});
		} else {
			cb();
...
Builder.prototype._executeScript = function() {
	var error = false
		, script, command, args;
	if (this.config.runtimeOptions.script && (script = this.config.settings.script)) {
		script = script.split(' ');
		command = script.shift();
		args = script;
		print('executing script...', 2);
		debug("execute: " + (strong(this.config.settings.script)), 3);
