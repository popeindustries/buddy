...
Builder.prototype.install = function(configpath, verbose) {
	// warn('the install command has been disabled awaiting downstream fixes for Node 0.10', 2);
	var self = this;
	term.start('install');
...
Builder.prototype.watch = function(configpath, options) {
	...
	this.build(configpath, options, function(err) {
		...
		if (self.config.runtimeOptions.reload || self.config.runtimeOptions.serve) {
			...
			serverfarm.start(self.config.runtimeOptions.serve, self.config.runtimeOptions.reload, settings, function(err) {
 ...
 * @param {File} file
 */
Builder.prototype._onFileCacheChange = function(err, file) {
	var self = this;
	// Don't throw
