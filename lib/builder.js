 ...
 * @param {Boolean} verbose
 */
Builder.prototype.watch = function(configpath, compress, reload, server, test, lazy, verbose) {
	var self = this;
	// Build first
	this.build(configpath, compress, false, test, lazy, verbose, function(err) {
		...
		// Start servers
		if (reload || server) {
			serverfarm.start(server, reload, self.config.settings.server || {directory: null, port: null}, function(err) {
