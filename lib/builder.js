...
var Builder, CSS, HTML, JS, RE_IGNORE_FILE, RE_WATCH_IGNORE_FILE, fs, path,
...
module.exports = Builder = (function() {
  ...
  Builder.prototype.initialize = function(configpath, fn) {
    if (!this.initialized) {
      this.config = new Configuration(configpath);
      return this.config.load(function(err) {
        if (err) {
  ...
  Builder.prototype._validFileType = function(type, filename) {
    var compiler, extension, name, _ref;
    extension = path.extname(filename).slice(1);
    if (extension === type) {
      return true;
    }
    _ref = this.plugins[type].compilers;
    for (name in _ref) {
      compiler = _ref[name];
  ...
  Builder.prototype._validBuildType = function(type) {
    var _ref;
    return (((_ref = this.config.build[type]) != null ? _ref.sources : void 0) != null) && this.config.build[type].sources.length >= 1 && (this.config.build[type].targets != null) && this.config.build[type].targets.length >= 1;
  };

  Builder.prototype._getFileType = function(filename) {
    var compiler, extension, name, type, _i, _len, _ref, _ref1;
    extension = path.extname(filename).slice(1);
    _ref = [JS, CSS];
  ...
  Builder.prototype._targetFactory = function(type, props, fn) {
    ...
    return fs.stat(inputpath, function(err, stats) {
      var dir, inSources, isDir, location, options, _i, _len, _ref;
      if (err) {
        return fn(err);
      } else {
      ...
      if (type === JS) {
        ...
        return fn(null, new JSTarget(inputpath, outputpath, this[type + 'Sources'], options));
      } else if (type === CSS) {
        return fn(null, new CSSTarget(inputpath, outputpath, this[type + 'Sources'], options));
      }
    });
  };
