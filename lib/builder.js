...
Builder.prototype._initialize = function (configpath, options) {
	...
	if (!this.initialized) {
		...
		return configuration.load(configpath, options)
			.then(function (data) {
				self.config = data;
				...
				// Create cache
				self.fileCache = fileFactory.cache = new FileCache(data.runtimeOptions.watch);
				...
				// Initialize targets
				self.targets = self._initializeTargets(data.build.targets);
...
Builder.prototype._buildTargets = function (targets) {
	...
	return targets.reduce(function (sequence, target) {
		// Chain
