...
module.exports = {
  ...
  _parseSourceFolder: function(dir) {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      if (!item.match(this.RE_IGNORE_FILE)) {
        ...
        if (fs.statSync(itempath).isDirectory()) this._parseSourceFolder(itempath);
        if (f = this._fileFactory(itempath)) {
  ...
  _fileFactory: function(filepath) {
    var contents;
