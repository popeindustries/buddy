 ...
 * @param {Function} fn
 */
Builder.prototype.watch = function (configpath, options, fn) {
	var self = this;
	...
	this.build(configpath, options, function (err, filepaths) {
		...
		if (self.config.runtimeOptions.reload || self.config.runtimeOptions.serve) {
			...
			// Start servers
			serverfarm.start(self.config.runtimeOptions.serve, self.config.runtimeOptions.reload, self.config.server, function (serverError) {
				if (serverError && !~serverError.message.indexOf('ECONNRESET')) {
					error(serverError, 2);
				}
			});
		} else {
...
Builder.prototype.init = function (configpath, options) {
	...
	if (!this.initialized) {
		...
		print('loaded config ' + strong(this.config.url), 1);
...
Builder.prototype.onFileCacheChange = function (file) {
	var now = new Date()
		...
		, targets = this.targets[0].targets.filter(function (target) {
