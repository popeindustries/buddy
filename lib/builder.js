 ...
 * @returns {Array}
 */
Builder.prototype.build = co(function* (configpath, options) {
	try {
		var filepaths = yield this._build(configpath, options);
		return filepaths;
	} catch (err) {
		return error(err, 2);
	}
...
Builder.prototype._build = function* (configpath, options) {
	cnsl.start('build');
