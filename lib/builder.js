...
Builder.prototype.build = function (configpath, options) {
	...
	// Build targets
	return this._buildTargets(this.targets)
	...
	this.build(configpath, options)
		.then(function (filepaths) {
			if (self.config.runtimeOptions.reload || self.config.runtimeOptions.serve) {
				...
				// Start servers
				serverfarm.start(self.config.runtimeOptions.serve, self.config.runtimeOptions.reload, self.config.server)
					.catch(function (err) {
