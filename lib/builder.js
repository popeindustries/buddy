...
process.on('uncaughtException', function(err) {
	require('buddy-term').error(err, 0, false);
	...
	this._initialize(configpath, function(err) {
		...
		if (self.config.dependencies) {
			...
			dependencies.install(self.config.dependencies, function(err, files) {
				...
				if (err) error(err, 0);
				print("completed install in " + (colour((+new Date() - start) / 1000 + 's', term.CYAN)), 2);
			});
		} else {
...
Builder.prototype.build = function(configpath, compress, lint, test, lazy, verbose, fn) {
	...
	this._initialize(configpath, function(err) {
		if (err) error(err, 2);
		async.forEachSeries([JS, CSS, HTML], (function(type, cb) {
 ...
 * @param {Boolean} verbose
 */
Builder.prototype.watch = function(configpath, compress, reload, serve, test, lazy, verbose) {
	var self = this;
	// Build first
	this.build(configpath, compress, false, test, lazy, verbose, function(err) {
		...
		// Start servers
		if (reload || serve) {
			serverfarm.start(serve, reload, self.config.settings.server || {directory: null, port: null}, function(err) {
		...
		[self.sources.js, self.sources.css, self.sources.html].forEach(function(source) {
			if (source) {
					...
					if (err) {
						...
						self._buildTargets(source.type, function(err) {
							...
							if (err) {
								...
								print("completed build in " + (colour((+new Date() - start) / 1000 + 's', term.CYAN)), 3);
								// Run test script
								if (self.options.test && (script = self.config.settings.test)) {
									self._executeScript(function(script, err) {
										if (err) return error(err, 2);
...
Builder.prototype._initialize = function(configpath, fn) {
	var self = this;
	term.verbose = this.options.verbose;
	start = new Date();
	if (!this.initialized) {
