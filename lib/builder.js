...
module.exports = Builder = (function() {
  ...
  Builder.prototype.compile = function(configpath) {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      if (this[type + 'Targets'].length) {
        ...
        _results.push(term.out("" + (term.colour('WARN [empty]', term.YELLOW)) + " no " + type + " build targets specified", 4));
      }
    }
    return _results;
  };
  ...
  Builder.prototype.watch = function(configpath) {
    var file, path, that, type, _i, _len, _ref, _results;
    if (!fs.watch) return;
    this.compile(configpath);
    that = this;
    _ref = [this.JS, this.CSS];
    _results = [];
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      if (this[type + 'Sources'].count) {
        ...
        _results.push((function() {
          ...
          for (path in _ref2) {
            file = _ref2[path];
            _results2.push(this._watchFile(file));
          }
          return _results2;
        }).call(this));
      } else {
  ...
  Builder.prototype.deploy = function(configpath) {
    return this.compile(configpath);
  };
  ...
  Builder.prototype._watchFile = function(file) {
    var stat, watcher;
    var _this = this;
    stat = fs.statSync(file.filepath);
    file.lastChange = +stat.mtime;
    file.lastSize = stat.size;
    return watcher = fs.watch(file.filepath, function(event) {
      var last, nstat, size;
      if (event === 'change') {
