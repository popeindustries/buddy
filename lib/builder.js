	...
	this.build(configpath, options, function (err, filepaths) {
		co(function* () {
			if (self.config.runtimeOptions.reload || self.config.runtimeOptions.serve) {
				...
				// Start servers
				try {
					yield* serverfarm.start(self.config.runtimeOptions.serve, self.config.runtimeOptions.reload, self.config.server);
				} catch (err) {
		...
		co(function * () {
			try {
				...
				if (serverfarm) {
					...
					// Restart app server
					if (servers.length) yield serverfarm.restart();
					// Refresh browser
