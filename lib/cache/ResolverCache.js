...
module.exports = class ResolverCache {
  ...
  setFile (file, versionDelimiter) {
    ...
    if (file.path && file.id && !this._fileCache.has(file.path) && !this._fileCache.has(file.id)) {
      ...
      if (!this._versionedFileCache.has(name)) {
        ...
        let versions = this._versionedFileCache.get(name);
   ...
   * @param {String} versionDelimiter
   */
  checkMultipleVersions (id, filepath, versionDelimiter) {
    ...
    warn(`multiple versions of the ${strong(pkg)} package will be bundled: ${strong(versions.join(', '))}`, 2);
    ...
    if (filepath.indexOf('node_modules') !== filepath.lastIndexOf('node_modules')) {
      ...
      print(`If possible, consider updating the ${strong(sourcePkg)} package to use version ${strong(canonicalVersion)}`, 2);
    } else {
      print(`Consider updating ${strong('package.json')} to use version ${strong(canonicalVersion)}`, 2);
    }
