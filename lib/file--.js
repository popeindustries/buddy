...
class File {
  ...
  load (options, fn) {
    if (!this.fileContent) {
      const encoding = (this.type == 'image' && this.extension != 'svg') ? null : 'utf8';
  ...
  compile (options, fn) {
    ...
    if (!this.compiledContent) {
      ...
      if (this.type == 'html') {
        ...
        this.getAllDependencies()
          .forEach((dependency) => {
            ...
            // Sidecar data file
              ...
              try {
                compilerOptions.data = JSON.parse(dependency.content);
              } catch (err) {
        ...
        compilerOptions.paths = factory.cache.getDirs();
      }

      helpers.compile(this, compilerOptions, (err, content) => {
        if (err) {
          // Don't quit on error
          error(err, 2, false);
          return fn();
  ...
  replaceEnvironment (options, fn) {
    this.content = helpers.replace.environment(this.content);
