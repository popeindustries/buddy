 ...
 * @returns {File}
 */
var factory = module.exports = function (filepath, isRoot, options) {
	var opts = clone(options)
		, type = filetype(filepath, isRoot, options.type, options.fileExtensions) || options.type
		, file, id;
	...
	if (factory.cache.hasFile(filepath)) {
		...
		// Create new
			...
			file = new File(id, filepath, type, isRoot, options);
 ...
 * @param {Object} options
 */
function File (id, filepath, type, isRoot, options) {
			...
			if (deps) {
				deps.forEach(function (dep) {
					...
					var filepath = resolve(self.filepath, dep.filepath, self.options)
						, instance = factory(filepath, false, self.options);
