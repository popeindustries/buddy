...
File.prototype.load = function (watching, fn) {
	if (!this.fileContent) {
		var content = fs.readFileSync(this.filepath, (this.type == 'image' && this.extension != 'svg') ? null : 'utf8');
...
File.prototype.compress = function (watching, fn) {
	...
	helpers.compress(this.type, this.content, function (err, content) {
		if (err) return fn(err);
		self.content = content;
		debug('compressed: ' + strong(self.relpath), 4);
		fn();
	});
};
