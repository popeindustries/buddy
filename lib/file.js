...
exports.JSFile = JSFile = (function() {
  ...
  JSFile.prototype.RE_MODULE = /^(?:require\.)?module\s*\(?\s*['|"].+module, ?exports, ?require\s*\)/m;
    ...
    if (this.RE_UPPERCASE.test(this.name)) {
      ...
      for (i = 0, _len = letters.length; i < _len; i++) {
        ...
        if (this.RE_UPPERCASE.test(letter)) {
          letters[i] = (i > 0 && letters[i - 1] !== '/' ? '_' : '') + letter.toLowerCase();
        }
      }
...
exports.CSSFile = CSSFile = (function() {
  ...
  CSSFile.prototype.RE_CSS_SRC_EXT = /\.styl$|\.less$/;
  ...
  function CSSFile(filepath, base) {
    ...
    this.compile = this.RE_CSS_SRC_EXT.test(this.filepath);
