...
/**
 * Parse and validate build 'target' data
 * @param {Object} target
 * @param {Object} config
 * @returns {Object}
 */
exports.parse = function (target, config) {
	config = config || extend({}, DEFAULT_CONFIG);

	var tgt = {};

	if (target.modular == null) tgt.modular = true;
	tgt.fileExtensions = config.fileExtensions;
	tgt.runtimeOptions = clone(config.runtimeOptions);
	if (target.input && target.output) {
		tgt.input = target.input;
		parseInput(tgt);
		tgt.output = target.output;
		tgt.output_compressed = target.output_compressed || '';
		parseOutput(tgt);

		var isInputArray = Array.isArray(tgt.input)
			, isOutputArray = Array.isArray(tgt.output)
			, isInputFile = !isInputArray && path.extname(tgt.inputPath).length
			, isOutputFile = !isOutputArray && path.extname(tgt.outputPath).length;

		if (isInputArray && isOutputArray) {
			if (tgt.input.length == tgt.output.length) {
				// Split
			} else {
			...
			tgt.concat = isOutputFile;
			tgt.batch = !isOutputFile;
