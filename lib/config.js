...
module.exports = {
  ...
  parse (targets, config) {
    return targets.reduce((parsed, target) => {
  ...
  validateInputOutput (target) {
    ...
    if (target.output) {
      ...
      if (isInputArray && isOutputArray) {
        ...
        throw Error(`total number of inputs (${strong(target.input)}) do not match total number of outputs (${strong(target.output)})`);
      } else if (!isInputArray && isOutputArray || !isInputFile && isOutputFile || isInputArray && isOutputFile) {
        throw Error(`unable to resolve inputs (${strong(target.input)}) with outputs (${strong(target.output)})`);
      } else if (isInputArray && !isOutputFile) {
        // Multiple => dir
        target.batch = true;
      }
    }
