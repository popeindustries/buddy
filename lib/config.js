 ...
 * @returns {Object}
 */
exports.load = function load (configpath, runtimeOptions) {
  ...
  config.plugins = exports.loadPlugins();
 ...
 * @returns {null}
 */
function registerPlugin (resource, plugins) {
    ...
    if (module.registration) {
      ...
      if (module.registration.extensions
        ...
        && 'function' == typeof module.compile) {
          ...
          for (const type in types) {
            ...
            exts.forEach((ext) => {
              ...
              if (!plugins.compilers[ext]) {
                // Store
                plugins.compilers[ext] = module;
                print('registered compiler plugin ' + chalk.bold.grey(module.registration.name) + ' for .' + ext, 2);
              }
            });
          }
        ...
        && 'function' == typeof module.compress) {
          ...
          // Don't overwrite if already registered
          if (!plugins.compressors[type]) {
            // Store
            plugins.compressors[type] = module;
            print('registered compressor plugin ' + chalk.bold.grey(module.registration.name) + ' for ' + type, 2);
          }
      } else {
