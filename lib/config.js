...
class Config {
  ...
  fileFactory (filepath, options) {
    ...
    // Retrieve cached
    if (caches.fileInstances.hasFile(filepath)) {
      file = caches.fileInstances.getFile(filepath);

    // Create new if valid id
    } else {
      const extension = path.extname(filepath).slice(1);
      const type = filetype(filepath, fileExtensions);
      const id = identify(filepath, { fileExtensions, sources, type });

      if (!id) throw Error(`unable to create file for: ${strong(filepath)}`);

      file = new this.fileDefinitionByExtension(extension)(id, filepath, type, options);
