...
function processWorkflows (runtimeOptions) {
	...
	workflows.js.push(['modular:output:inline', 'modular:output:concat']);
	...
	if (target.input) {
		...
		target.input = flatten(target.input.map(function (input) {
			...
			if (runtimeOptions.grep) {
				include = match(input, runtimeOptions.grep, { matchBase: true });
...
function validateInputOutput (target) {
	if (target.input) {
		...
		if (target.output) {
			var isOutputArray = Array.isArray(target.output)
