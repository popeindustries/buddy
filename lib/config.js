...
module.exports = {
  ...
  parseOutput (target, fileExtensions, sources, runtimeOptions) {
    if (target.output) {
      ...
      target.outputpaths = inputpaths.map((input, idx) => {
        ...
        return isDirectory
          ? this.resolveOutput(input, outputpath, target.batch, fileExtensions, sources)
          : outputpath;
      });
    } else {
      target.outputpaths = [];
  ...
  /**
   * Resolve directory 'output' based on 'input'
   * @param {String} input
   * @param {String} output
   * @param {Boolean} batch
   * @param {Object} fileExtensions
   ...
   * @returns {String}
   */
  resolveOutput (input, output, batch, fileExtensions, sources) {
    ...
    if (batch) {
      for (let i = sources.length - 1; i > 0; i--) {
        if (~input.indexOf(sources[i])) {
          outputpath = path.join(output, path.relative(sources[i], input));
          break;
        }
      }
    }
