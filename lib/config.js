...
module.exports = {
  ...
  parse (targets, config) {
    return targets.reduce((targets, target) => {
      ...
      if (!('input' in target) || target.input != null) {
        this.parseOutput(target, config.fileExtensions, config.sources, config.runtimeOptions);
  ...
  parseInput (target, fileFilter, runtimeOptions) {
    if (target.input) {
      ...
      target.inputpaths = flatten(inputs.map((input) => {
        // Expand glob pattern
        ...
        if (!path.extname(input).length) {
          input = readdir(input, (resource, stat) => {
            return stat.isFile();
          });
        }
        ...
        } else if (!fileFilter.test(input)) {
          include = false;
        }

        return include;
      }).reduce((inputpaths, input) => {
      ...
      if (!target.inputpaths.length) {
        ...
        target.batch = target.inputpaths.length > 1;
      }
    }
  },
   ...
   * @param {Object} runtimeOptions
   */
  parseOutput (target, fileExtensions, sources, runtimeOptions) {
    if (target.output) {
      ...
      target.outputpaths = inputpaths.map((input, idx) => {
        const outputpath = path.resolve(isDirectory ? outputs[0] : outputs[idx]);

        return isDirectory
          ? this.resolveOutput(input, outputpath, fileExtensions, sources)
          : outputpath;
      });
    }
  },
  ...
  /**
   * Resolve 'output' based on 'input' when directory
   * @param {String} input
   * @param {String} output
   * @param {Object} fileExtensions
   * @param {Array} sources
   * @returns {String}
   */
  resolveOutput (input, output, fileExtensions, sources) {
    // TODO: determine correct path based on sources
    let outputpath = input.replace(path.dirname(input), output);
    const extension = path.extname(outputpath)
      , type = filetype(input, fileExtensions);
