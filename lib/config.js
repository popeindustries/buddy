...
module.exports = {
  ...
  load (configpath, runtimeOptions) {
    ...
    // Load compilers (+ fileExtensions) and compressors
    plugins.buddy(config);
  ...
  loadConfig (configpath, config) {
    ...
    if (!data.build && data.input) data = { build: data };
  ...
  parseBuild (build, config) {
    ...
    return build.reduce((build, buildTarget) => {
      ...
      if (buildTarget.options == null) buildTarget.options = {};
      if (buildTarget.sources) {
        config.sources = config.sources.concat(buildTarget.sources);
        delete buildTarget.sources;
      }
      buildTarget.index = num++;
      buildTarget.isRoot = !buildTarget.input;
      ...
      if (!('input' in buildTarget) || buildTarget.input != null) {
        ...
        if (buildTarget.build) {
          ...
          buildTarget.build = this.parseBuild(buildTarget.build, config).map((childBuildTarget, idx) => {
            num += idx;
            childBuildTarget.hasParent = !buildTarget.isRoot;
            childBuildTarget.index = num;
            return childBuildTarget;
          });
  ...
  parseAppServer (buildTarget, config) {
    ...
    if (config.server && config.server.file) {
      buildTarget.appServer = contains(buildTarget.inputpaths, path.resolve(config.server.file));
    }
  },
