...
module.exports = {
  ...
  processWorkflows (config) {
    config.workflows = {
      css: {
        std: [['load'], [], ['output:inline', 'output:compile', 'output:concat']],
        inline: [['load'], [], ['output:inline', 'output:compile', 'output:concat']]
      },
      html: {
        std: [['load', 'parse', 'output:replaceReferences'], [], ['output:compile', 'output:inline']],
        inline: [['load', 'parse', 'output:replaceReferences'], [], []]
      },
      js: {
        std: [['load'], [], []],
        inline: [['load'], [], []]
    ...
    // CSS
    if (config.runtimeOptions.lint) {
      config.workflows.css.std[0].push('lint');
      config.workflows.css.inline[0].push('lint');
    }
    config.workflows.css.std[0].push('parse');
    config.workflows.css.inline[0].push('parse');
    // Allow inline-source to handle compression
    if (config.runtimeOptions.compress) config.workflows.css.std[2].push('output:compress');

    // JS
    if (config.runtimeOptions.lint) {
      config.workflows.js.std[0].push('lint');
      config.workflows.js.inline[0].push('lint');
    }
    // Allow for dead code removal by replacing env first
    config.workflows.js.std[0].push('output:replaceEnvironment', 'output:compile', 'parse', 'modular:output:replaceReferences');
    config.workflows.js.inline[0].push('output:replaceEnvironment', 'output:compile', 'parse', 'modular:output:replaceReferences');
    if (config.runtimeOptions.lazy) {
      if (config.runtimeOptions.compress) config.workflows.js.std[0].push('modular:output:compress');
      config.workflows.js.std[0].push('modular:output:escape');
    }
    config.workflows.js.std[0].push('modular:output:wrap');
    config.workflows.js.std[1].push('output:inline');
    config.workflows.js.inline[1].push('output:inline');
    config.workflows.js.std[2].push('modular:output:concat');
    if (config.runtimeOptions.compress) config.workflows.js.std[2].push('modular:output:compress');

    // IMAGE
    if (config.runtimeOptions.compress) {
      config.workflows.image.std[2].push('output:compress');
    }
  },
