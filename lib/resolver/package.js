...
function getDetails (filepath, options) {
  ...
  if (!pkgpath.includes('node_modules') && !fs.existsSync(manifestpath)) {
  ...
  if (cache.getPackage(details.id)) details.id += versionDelimiter + details.version;
...
function resolvePath (filepath) {
  ...
  if (filepath.includes(cwd)) {
    ...
    while (true) {
      ...
      if (dir == cwd || fs.existsSync(path.resolve(dir, 'node_modules'))) return dir;
...
function resolveId (details, filepath) {
  ...
  if ('string' == typeof filepath) {
    ...
    const version = details.id.includes(versionDelimiter)
      ? versionDelimiter + details.version
      : '';
