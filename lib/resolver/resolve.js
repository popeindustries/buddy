...
const { resolverCache: cache } = require('../cache');
...
module.exports = function resolve (sourcepath, id, options) {
  ...
  const type = filepathType(sourcepath, options.fileExtensions);
...
function find (id, type, sourcedir, options) {
  ...
  if (isAbsoluteFilepath(filepath)) {
    filepath = findFilepath(filepath, type, options.fileExtensions);
    filepath = alias.resolve(filepath, pkgDetails && pkgDetails.aliases);
