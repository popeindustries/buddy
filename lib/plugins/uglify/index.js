...
function extend (prototype, utils) {
  ...
  prototype.compress = function compress (buildOptions, fn) {
    try {
      ...
      warn(`unable to compress ${strong(this.relpath)} with Uglify. Make sure the build version is set to ${strong('es5')} as Uglify doesn't yet support newer language features`, this.options.level);
    }
    fn();
  };
}