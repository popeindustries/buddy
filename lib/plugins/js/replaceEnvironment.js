 ...
 * @returns {String}
 */
module.exports = function replaceEnvironment (content, map, browser = true) {
  let matches = {};
  let match;
  ...
  for (const context in matches) {
    ...
    const re = new RegExp(regexpEscape(context), 'gm');

    content = content.replace(re, matches[context]);
    if (map) map.replaceRight(re, matches[context]);
  }
