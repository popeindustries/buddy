...
module.exports = function concat(file, headerComment, buildOptions) {
  ...
  // Add wrapper open
  if (browser) file.appendContent(bootstrap ? '(function () {' : `$m['${file.id}'] = function () {`);
  ...
  if (!file.isDummy) file.appendContent(header);
  ...
  if (!file.isDummy) file.appendContent(footer);
  ...
  if (helpers) {
    ...
    if (helpers.length)
      // Don't clobber existing if multiple bundles define helpers
      headerContent += `${buildExternalHelpers(helpers).replace(
