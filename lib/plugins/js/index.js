  ...
  return class JSFile extends File {
    ...
    load (buildOptions, fn) {
      super.load(buildOptions, (err) => {
        ...
        this.map = new SourceNode(null, null, null, this.content);
    ...
    transpile (buildOptions, fn) {
      ...
      [...this.getAllDependencies(), this].some((file) => {
        ...
        const options = Object.assign({}, babelOptions, {
          ...
          sourceMaps: true,
          sourceMapTarget: file.relpath,
          sourceRoot: this.options.webroot
        });
        ...
        if (transpiledFingerprint == file.transpiledFingerprint) {
          ...
          try {
            const transpiled = transpile(file.content, options);
    ...
    replaceReferences (buildOptions, fn) {
      [...this.getAllDependencies(), this].forEach((file) => {
        ...
        const replaced = replaceReferences(file.content, file.map, file.dependencyReferences);

        file.content = replaced.content;
        file.map = replaced.map;
    ...
    inline (buildOptions, fn) {
      [...this.getAllDependencies(), this].forEach((file) => {
        ...
        const inlined = inline(file.content, file.map, file.dependencyReferences, buildOptions.browser);

        file.content = inlined.content;
        file.map = inlined.map;
    ...
    replaceDynamicReferences (buildOptions, fn) {
      [this, ...this.getAllDependencies()].forEach((file) => {
        ...
        const replaced = replaceDynamicReferences(file.content, file.map, buildOptions.browser, file.dynamicDependencyReferences);
    ...
    concat (buildOptions, fn) {
      const concated = concat(this, HEADER, buildOptions);

      this.content = concated.content;
      this.map = concated.map;
