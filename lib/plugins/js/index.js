...
function define (File, utils) {
  return class JSFile extends File {
    ...
    load (buildOptions, fn) {
      super.load(buildOptions, (err) => {
        ...
        if (buildOptions && buildOptions.bundle && RE_STRICT.test(this.content)) {
          this.string = new MagicString(this.content.replace(RE_STRICT, ''));
        }
    ...
    parse (buildOptions, fn) {
      ...
      const [requires, imports, dynamicImports] = parse(this.string.original);
    ...
    replaceEnvironment (buildOptions, fn) {
      [...this.getAllDependencies(), this].forEach((file) => {
        replaceEnvironment(file.string, buildOptions.browser);
    ...
    inline (buildOptions, fn) {
      [...this.getAllDependencies(), this].forEach((file) => {
        ...
        inline(file.string, file.dependencyReferences, buildOptions.browser);
    ...
    transpile (buildOptions, fn) {
      ...
      [...this.getAllDependencies(), this].some((file) => {
        ...
        if (transpiledFingerprint == file.transpiledFingerprint && file.transpiledString) {
          file.string = file.transpiledString;
        } else {
          try {
            file.string = file.transpiledString = new MagicString(transpile(file.string.toString(), options));
    ...
    reparse (buildOptions, fn) {
      [this, ...this.getAllDependencies()].forEach((file) => {
        ...
        const [requires, imports, dynamicImports] = parse(file.string.original);
    ...
    replaceReferences (buildOptions, fn) {
      [...this.getAllDependencies(), this].forEach((file) => {
        if (file.isBuddyBuilt || file.isBuilt) return;
        replaceReferences(file.string, file.dependencyReferences);
    ...
    replaceDynamicReferences (buildOptions, fn) {
      [this, ...this.getAllDependencies()].forEach((file) => {
        ...
        replaceDynamicReferences(file.string, buildOptions.browser, file.dynamicDependencyReferences);
    ...
    concat (buildOptions, fn) {
      this.string = concat(this, HEADER, buildOptions);
    ...
    reset (hard) {
      ...
      this.isCircularDependency = false;
      if (!hard) {
        this.transpiledString = this.string;
      } else {
        this.transpiledFingerprint = '';
        this.transpiledString = null;
      }
