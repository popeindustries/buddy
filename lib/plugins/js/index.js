  ...
  return class JSFile extends File {
    ...
    load (buildOptions, fn) {
      super.load(buildOptions, (err) => {
        ...
        this.isBuddyBuilt = RE_BUDDY_BUILT.test(this.fileContent);
        this.isBuilt = RE_UMD_BUILT.test(this.fileContent);

        if (this.isBuddyBuilt) concat.unwrap(this, buildOptions);
    ...
    parse (buildOptions, fn) {
      ...
      const content = commentStrip(this.content.toString());
    ...
    transpile (buildOptions, fn) {
      ...
      [...this.getAllDependencies(), this].some((file) => {
        ...
        if (transpiledFingerprint == file.transpiledFingerprint) {
          ...
          try {
            const transpiled = transpile(file.content.toString(), options);
