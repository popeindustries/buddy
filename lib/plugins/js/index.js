      ...
      [...this.getAllDependencies(), this].some((file) => {
        ...
        if (transpiledFingerprint == file.transpiledFingerprint && file.transpiledContent) {
          ...
          try {
            ...
            const result = transpile(file.content, options);

            file.content = file.transpiledContent = result.code;
            file.map = SourceMapGenerator.fromSourceMap(new SourceMapConsumer(result.map));
            file.map.setSourceContent(file.relpath, file.content);
