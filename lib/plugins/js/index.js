...
function define(File, utils) {
  return class JSFile extends File {
    ...
    hashContent(content, forWrite = false) {
      if (forWrite) console.log(this.id, this.dynamicDependencyReferences)
      return forWrite
        ? md5(
            [
              content,
              ...this.getAllDependencies().map(file => file.content),
              ...this.dynamicDependencyReferences.map(ref => ref.file.writeHash)
            ].join('')
          )
        : md5(content);
    }
