  ...
  return class JSFile extends File {
    ...
    load (buildOptions, fn) {
      super.load(buildOptions, (err) => {
        ...
        if (buildOptions && buildOptions.bundle && RE_STRICT.test(this.content)) {
          this.content = this.content.replace(RE_STRICT, '');
        }
    ...
    replaceEnvironment (buildOptions, fn) {
      [...this.getAllDependencies(), this].forEach((file) => {
        ...
        file.content = replaceEnvironment(file.content, buildOptions.compress ? true : buildOptions.browser);
    ...
    transpile (buildOptions, fn) {
      ...
      [...this.getAllDependencies(), this].some((file) => {
        ...
        if (transpiledFingerprint == file.transpiledFingerprint && file.transpiledContent) {
          ...
          try {
            // options.inputSourceMap = file.map.toJSON();
            const { code: content/*, map*/ } = transpile(file.content, options);

            file.content = file.transpiledContent = content;
            // file.map = SourceMapGenerator.fromSourceMap(new SourceMapConsumer(map));
            // file.map.setSourceContent(file.relpath, file.fileContent);
    ...
    concat (buildOptions, fn) {
      // Convert to SourceNodes
      // [this, ...this.getAllDependencies()].forEach((file) => {
      //   if (file.map && file.map instanceof SourceMapGenerator) {
      //     file.map = SourceNode.fromStringWithSourceMap(file.content, new SourceMapConsumer(file.map.toString()));
      //     file.map.setSourceContent(file.relpath, file.fileContent);
      //   }
      // });

      const { content/*, map*/ } = concat(this, HEADER, buildOptions);

      this.content = content;
      // Convert back to SourceMapGenerator
      // this.map = map.toStringWithSourceMap({ file: path.relative(process.cwd(), this.writepath) }).map;
