...
const { identify, resolve } = require('../../resolver');
  ...
  return class JSFile extends File {
    ...
    addDynamicDependencies (dependencies) {
      ...
      dependencies.forEach((dependency) => {
        ...
        dependency.file = fileFactory(filepath, this.options);
        this.dynamicDependencyReferences.push(dependency);
        if (this.options.buildFactory) this.options.buildFactory(filepath);
      });
    ...
    replaceDynamicReferences (buildOptions, fn) {
      ...
      const baseurl = `/${path.relative(webroot, path.dirname(this.writepath))}`;

      [this, ...this.getAllDependencies()].forEach((file) => {
        if (file.isBuilt) return;

        file.content = replaceDynamicReferences(file.content, baseurl, file.dynamicDependencyReferences);
