...
const WORKFLOW_STANDARD = ['load', 'parse', 'compile', 'replaceEnvironment', 'transpile', 'runForDependencies'];
  ...
  return class JSFile extends File {
    ...
    constructor(id, filepath, options) {
      ...
      */
      ...
      this.shouldTranspile = this.isNpmModule ? RE_TRANSPILE.test(this.content) : true;
    ...
    parse(buildOptions, fn) {
      ...
      const [requires, imports, dynamicImports] = this.parseDependencyReferences(this.content);
    ...
    reparse(buildOptions, fn) {
      ...
      [this, ...this.getAllDependencies()].forEach(file => {
        ...
        const [requires, imports, dynamicImports] = this.parseDependencyReferences(file.content);
