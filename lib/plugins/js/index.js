  ...
  return class JSFile extends File {
    ...
    load (buildOptions, fn) {
      super.load(buildOptions, (err) => {
        ...
        if (this.isBuddyBuilt) this.string = new MagicString(concat.unwrap(this.content, this.id));
    ...
    transpile (buildOptions, fn) {
      ...
      [...this.getAllDependencies(), this].some((file) => {
        ...
        const contentFingerprint = md5(JSON.stringify(file.content.toString()));
        ...
        if (transpiledFingerprint != file.transpiledFingerprint) {
          try {
            file.string = new MagicString(transpile(file.string.toString(), options));
