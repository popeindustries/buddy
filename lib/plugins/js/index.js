...
function define (File, utils) {
  return class JSFile extends File {
    ...
    parse (buildOptions, fn) {
      ...
      super.addDependencies(requires.concat(imports)
        .map((match) => {
      ...
      // Create child builds for dynamic imports
      dynamicImports.forEach((match) => {
        match.id = match.match;

        const filepath = resolve(this.filepath, match.id, { fileExtensions: this.options.fileExtensions });

        // Unable to resolve filepath or create instance
        if (filepath === '') {
          warn(`dependency ${strong(match.id)} for ${strong(this.relpath)} not found`, 3);
          return;
        }

        this.dynamicDependencyReferences.push(match);
        this.options.buildFactory(filepath);
      });
