  ...
  return class JSFile extends File {
    ...
    load (buildOptions, fn) {
      super.load(buildOptions, (err) => {
        ...
        this.map = new SourceNode(1, 0, this.filepath, this.content, this.id);
        this.map.setSourceContent(this.filepath, this.content);
    ...
    replaceEnvironment (buildOptions, fn) {
      [...this.getAllDependencies(), this].forEach((file) => {
        file.content = replaceEnvironment(file.content, file.map, buildOptions.browser);
    ...
    transpile (buildOptions, fn) {
      ...
      [...this.getAllDependencies(), this].some((file) => {
        ...
        const options = Object.assign({}, babelOptions, {
          inputSourceMap: file.map.toStringWithSourceMap().map.toString(),
          plugins,
          sourceMaps: true
        });
