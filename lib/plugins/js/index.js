  ...
  return class JSFile extends File {
    ...
    reparse(buildOptions, fn) {
      ...
      [this, ...this.getAllDependencies()].forEach(file => {
        ...
        const referencesById = file.dependencyReferences.reduce(
          (referencesById, dependency) => {
            referencesById[dependency.id] = dependency;
            return referencesById;
          },
          {}
        );
        const dynamicReferencesById = file.dynamicDependencyReferences.reduce(
          (dynamicReferencesById, dependency) => {
