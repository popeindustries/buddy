  ...
  return class JSFile extends File {
    ...
    transpile (buildOptions, fn) {
      [...this.getAllDependencies(true), this].forEach((file) => {
        ...
        if (!file.transpiled && !file.isBuddyBuilt && !file.isBuilt && !file.isNpmModule) {
          try {
            transpile(file, file.options.pluginOptions.babel);
    ...
    flatten (buildOptions, fn) {
      [...this.getAllDependencies(true), this].forEach((file) => {
        // Non-Buddy built files should be flattened
        if (file.flattened || file.isBuddyBuilt) return;

        try {
          // Do not replace 'module.exports' for writeable Node files
          flatten(file, buildOptions.browser ? true : !file.isWriteable(buildOptions.batch));

          debug(`flatten: ${strong(file.relpath)}`, 4);
          file.flattened = true;
        } catch (err) {
          return fn(err);
        }
