    ...
    visitor: {
      Program(path, state) {
        ...
        for (const binding in scope.bindings) {
          if (binding !== 'module' || binding !== 'exports') {
            console.log(binding, scope.bindings[binding].referenced, scope.bindings[binding].path.isReferenced())
            // renameRootDeclarations(scope.bindings[binding].path, opts.source.namespace);
          }
        }
      },
