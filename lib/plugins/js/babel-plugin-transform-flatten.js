...
function renameRootDeclarations (types, path, namespace) {
  ...
  if (oldName in scope.uids && !scope.getBinding(oldName)) {
...
function isReplaceableIdentifier (name, path, rootScope, includeGlobal) {
  ...
  if (!isRootScope && parent.isFunction()) {
  ...
  if (!includeGlobal && !isReallyRootScope(path.scope.getBinding(name), rootScope)) return false;
