...
class Target {
  ...
  */
  ...
  process (files, workflows, watching, fn) {
    ...
    function run (files, workflow, fileFilter, options, done) {
      if (!files.length) done(null, files);
      ...
      parallel(files.map((file) => {
        ...
        if (files.length) return run(files, workflow, fileFilter, options, done);

        done(null, files);
      });
    }

    function filter (runFiles) {
      return difference(distinct(compact(flatten(runFiles))), files);
    }
    ...
    waterfall(workflows.map((workflow, idx) => {
      return (runFiles, done) => {
        ...
        run(files, this.parseWorkflow(workflow.std), filter, options, done);
      };
    }), (err, files) => {
