...
class Target {
  ...
  */
    ...
    if (this.isRoot) {
      ...
      (done) => {
        series(this.targets.map((target) => {
          return apply(target.build, target);
        }), (err, results) => {
    ...
    // Skip if watch only and not watch build
      ...
      (files, done) => {
        parallel(files.map((file) => {
          return apply(this.executeHook, 'afterEach', file);
        }), (err) => {
          done(err, files);
        });
      },
  ...
  process (files, workflows, watching, fn) {
    // Recursive batch run 'files'
    function run (files, runningFiles, workflow, options, done) {
      // Default runningFiles is files
      if ('function' != typeof done) {
        done = options;
        options = workflow;
        workflow = runningFiles;
        runningFiles = files;
      }
      if (!runningFiles.length) done(null, files);

      parallel(runningFiles.map((file) => {
        // Memoize
        ...
        dependantFiles = difference(distinct(compact(flatten(dependantFiles))), files);
        if (dependantFiles.length) {
          // Store
          files = files.concat(dependantFiles);
          return run(files, dependantFiles, workflow, options, done);
        }
    ...
    waterfall(workflows.map((workflow, idx) => {
      return (runFiles, done) => {
        // Store
        this.referencedFiles = this.referencedFiles.concat(runFiles);
        if (idx == workflow.length - 2) {
          runFiles = runFiles.filter((file) => {
            // Don't filter project files if non-modular batch job
            return file.getIsWriteable(!this.modular && this.batch);
          });
        }
        run(runFiles || files, this.parseWorkflow(workflow), options, done);
      };
    }), (err, files) => {
