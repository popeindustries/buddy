var CSSTarget, JSTarget, Target, coffee, fs, growl, less, log, path, term, uglify;
...
exports.JSTarget = JSTarget = (function() {
    ...
    if (file.dependencies.length) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (dep = this.sourceCache.byModule[dependency] || this.sourceCache.byModule["" + dependency + "/index"]) {
          ...
          term.out("" + (term.colour('error', term.RED)) + " dependency " + (term.colour(dependency, term.GREY)) + " for " + (term.colour(this.module, term.GREY)) + " not found", 4);
        }
      }
    }
  ...
  JSTarget.prototype._build = function(mini, bare) {
    ...
    if (this.concatenate) {
      ...
      compiled = this._addHeader(compiled);
      term.out("" + (term.colour('compiled', term.GREEN)) + " " + (term.colour(path.basename(this.output), term.GREY)), 4);
      if (mini) {
      ...
      for (_j = 0, _len2 = _ref2.length; _j < _len2; _j++) {
        ...
        if (file.compile) {
          compiled = this._compile((bare ? file.contents : file.contentsModule), filepath);
          if (!compiled) return null;
          fs.writeFileSync(filepath, compiled, 'utf8');
          _results.push(term.out("" + (term.colour('compiled', term.GREEN)) + " " + (term.colour(path.basename(filepath), term.GREY)), 4));
        } else {
  ...
  JSTarget.prototype._compile = function(contents, name) {
    var compiled;
    try {
  ...
  JSTarget.prototype._minify = function(file, contents) {
    ...
    fs.writeFileSync(file, this._addHeader(compressed));
    return term.out("" + (term.colour('minified', term.GREEN)) + " " + (term.colour(path.basename(file), term.GREY)), 4);
  };
