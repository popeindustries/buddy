  ...
  */
  ...
  build (fn) {
    ...
    // Skip if watch only and not watch build
      ...
      // Process files
      (files, done) => {
        this.process(files, this.options.workflows, watching, (err, referencedFiles, writeableFiles) => {
          if (err) return done(err);
          this.referencedFiles = referencedFiles;
          done(null, writeableFiles);
        });
      },
  ...
  process (files, workflows, watching, fn) {
    ...
    const writeableFlag = !this.modular && this.batch;
    ...
    let options = {
      ...
      , processedFiles = files.slice()
      , writeableFiles = [];

    // Recursively run 'files' through 'workflow'
    function runWorkflow (files, workflow, options, done) {
      if (!files.length) return done();

      parallel(files.map((file) => {
        // Memoize
        return file.run.bind(file, workflow[file.type], options);
      }), (err, files) => {
        if (err) return done(err);
        runWorkflow(filterFiles(files), workflow, options, done);
      });
    }

    // Filter and store additional runnable files
    function filterFiles (files) {
      const runnableFiles = difference(distinct(flatten(files)), processedFiles);

      if (runnableFiles.length) processedFiles = processedFiles.concat(runnableFiles);
    ...
    function parseWorkflow (workflow) {
      ...
      for (const type in workflow) {
        parsed[type] = workflow[type].reduce((workflow, task) => {
          if (!~task.indexOf(':')) {
            workflow.push(task);
          // Conditional task
          } else {
    ...
    function process (workflows, done) {
      waterfall(workflows.map((workflow, idx) => {
        return (done) => {
          let files = processedFiles;

          // Filter only writeable files on last workflow
          if (idx == workflows.length - 1) {
            writeableFiles = files.filter((file) => {
              return file.getIsWriteable(writeableFlag);
            });
            files = writeableFiles;
          }
          runWorkflow(files, parseWorkflow(workflow), options, done);
        };
      }), done);
    }

    process(workflows.std, (err) => {
      if (err) return fn(err);
      fn(null, processedFiles, writeableFiles);
    });
  }
