 ...
 * @returns {Target}
 */
module.exports = function targetFactory (data) {
...
class Target {
  ...
  */
    ...
    if (this.fileExtensions) {
      ...
      for (const type in this.fileExtensions) {
  ...
  build (fn) {
    ...
    // Skip if watch only and not watch build
      ...
      // Parse source files
      (done) => {
        done(null, this.parse(this.batch, this.inputpath, this.fileFilter, this.fileFactoryOptions));
      },

      // Process files
      (files, done) => {
        this.process(files, this.workflows, watching, done);
      },
  ...
  parse (batch, input, filter, options) {
    ...
    if (batch) {
      ...
      const inputs = !Array.isArray(input) ? [input] : input
        , filepaths = flatten(inputs.map((inpt) => {
            // Directory
            if (!path.extname(inpt).length) {
              return readdir(inpt, (resource, stat) => {
