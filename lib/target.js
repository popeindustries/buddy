var CSSTarget, JSTarget, Target, coffee, file, fs, growl, less, log, path, term, uglify;
  ...
  Target.prototype._parseInputs = function(input) {
    ...
    if (fs.statSync(input).isFile()) {
      if (file = this.sourceCache.byPath[input]) {
        ...
        return term.out("" + (term.colour('warning', term.YELLOW)) + " " + (term.colour(this.input, term.GREY)) + " not found in sources", 2);
      }
    } else {
...
exports.JSTarget = JSTarget = (function() {
  ...
  JSTarget.prototype._build = function() {
    var content, contents, file, filepath, _i, _j, _len, _len2, _ref, _ref2, _results;
    if (this.concatenate) {
      contents = [];
      if (!this.nodejs) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        file = _ref[_i];
        contents.push(file.contentsModule);
      }
      ...
      for (_j = 0, _len2 = _ref2.length; _j < _len2; _j++) {
        file = _ref2[_j];
        filepath = path.join(this.output, file.name) + this.EXTENSION;
        content = this.nodejs ? file.contents : file.contentsModule;
        if (file.compile) {
          _results.push(this._compile(content, filepath, false));
        } else {
          _results.push(this._writeFile(content, filepath, false));
        }
      }
      return _results;
    }
  ...
  JSTarget.prototype._compile = function(content, filepath, header) {
    ...
    try {
      ...
      this._displayNotification("error building " + filepath + ": " + error);
      return null;
    }
  };
  ...
  JSTarget.prototype._compress = function(filepath, contents) {
...
exports.CSSTarget = CSSTarget = (function() {
  ...
  CSSTarget.prototype._build = function() {
    var file, filepath, _i, _len, _ref;
    _ref = this.sources;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      file = _ref[_i];
      filepath = path.join(this.output, file.name) + this.EXTENSION;
      this._makeDirectory(filepath);
      if (file.compile) {
        return this._compile(file.contents, filepath, path.extname(file.filepath));
      } else {
        return this._writeFile(file.contents, filepath);
      }
    }
  };
  ...
  CSSTarget.prototype._compile = function(content, filepath, extension) {
    ...
    if (file.CSSFile.prototype.RE_STYLUS_EXT.test(extension)) {
      stylc = stylus(content).set('paths', this.sourceCache.concat());
