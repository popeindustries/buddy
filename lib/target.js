...
Target.prototype.build = function (fn) {
	...
	// Skip if watch only and not watch build
	} else if (this.outputpath || !this.outputpath && this.runtimeOptions.watch) {
		var watching = this.runtimeOptions.watch ? 1 : 0
		...
		// Parse source files
		function (done) {
			done(null, self.parse(self.batch, self.inputpath, self.fileFilter, self.fileFactoryOptions));
		},

		// Process files
		function (files, done) {
			self.process(files, self.workflows, watching, done);
		},
...
Target.prototype.parse = function (batch, input, filter, options) {
	...
	if (batch) {
		...
		var inputs = !Array.isArray(input) ? [input] : input
			, filepaths = flatten(inputs.map(function (inpt) {
					// Directory
					if (!path.extname(inpt).length) {
						return readdir(inpt, function (resource, stat) {
 ...
 */
Target.prototype.process = function (files, workflows, watching, fn) {
	var self = this;
	...
	function batch (someFiles, idx, done) {
		parallel(someFiles.map(function (file) {
			// Memoize
			return file.run.bind(file, self.parseWorkflow(workflows[file.type][idx]), watching);
	...
	batch(files, 0, function (err) {
		...
		batch(files, 1, function (err) {
			if (err) return fn(err);

			// Filter writeable files
			files = files.filter(function (file) {
				// Don't filter project files if non-modular batch job
				return file.getIsWriteable(!self.modular && self.batch);
			});

			// Last set only operates on writeable files
			if (files.length) {
				batch(files, 2, fn);
			} else {
				fn(null, files);
 ...
 * @returns {Array}
 */
Target.prototype.parseWorkflow = function (workflow) {
	var self = this;
	...
	return files.map(function (file) {
		...
		if (this.outputpath) {
			...
			if (!path.extname(filepath).length) {
				var outputInSources = this.sources.some(function (source) {
