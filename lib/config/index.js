...
class Config {
  ...
  constructor (configpath, runtimeOptions) {
    ...
    if (configpath == null || 'string' == typeof configpath) {
      ...
      const isNamed = configpath
        && !path.extname(configpath)
        && !exists(path.resolve(configpath));
  ...
  fileFactory (filepath, options) {
    ...
    const versions = resolverCache.getFileVersions(id, versionDelimiter);

    if (versions.length > 1 && options.bundle && options.browser) {
...
function locateConfig (url) {
  ...
  function check (dir) {
    ...
    if (exists(urlFinal = urljs)
      || exists(urlFinal = urljson)
      || exists(urlFinal = urlpkgjson)) {
        return urlFinal;
    }
    return '';
  }
