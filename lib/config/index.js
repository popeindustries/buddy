...
const { cache: dependencyResolverCache, identify, VERSION_DELIMITER } = require('../dependency-resolver');
...
class Config {
  ...
  fileFactory (filepath, options) {
    const { fileCache, fileExtensions } = options;
    ...
    const id = identify(filepath, { fileExtensions });
    ...
    const versions = dependencyResolverCache.getFileVersions(id);
    ...
    if (versions.length > 1) {
      ...
      warn(`more than one version of the ${strong(pkg)} package exists (${strong(file.relpath)}). If ${strong(pkg)} is a project dependency, consider using the same version (${strong(version)}) to reduce duplication`, 2);
    }
