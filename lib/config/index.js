...
const { identify } = require('../dependency-resolver');
const buildParser = require('./buildParser');
const dependencyResolverCache = require('../dependency-resolver/cache');
...
class Config {
  ...
  constructor (configpath, runtimeOptions) {
    ...
    this.fileCache = fileCache(this.runtimeOptions.watch);
  ...
  fileFactory (filepath, options) {
    const { fileCache, fileExtensions, globalAliases } = options;

    // Retrieve cached
    if (fileCache.hasFile(filepath)) return fileCache.getFile(filepath);

    const extension = path.extname(filepath).slice(1);
    const id = identify(filepath, { fileExtensions, globalAliases });
    ...
    if (dependencyResolverCache.hasMultipleVersions(id)) {
      warn(`more than one version of ${strong(id.split('#')[0])} exists (${strong(file.relpath)})`, 2);
    }
