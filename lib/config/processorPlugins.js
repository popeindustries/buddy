...
module.exports = {
  ...
  parsePlugins(type = '', version = [], options = {}, compress = false) {
    options = merge({ buddy: { plugins: [] }, babel: { presets: [], plugins: [] }, postcss: { plugins: [] } }, options);
    ...
    if (type !== 'css') {
      let babelEnv = options.presets.find(preset => preset[0] === 'env');

      if (isNullOrUndefined(babelEnv)) {
        babelEnv = ['env', {}];
        options.presets.push(babelEnv);
      }

      Object.assign(babelEnv, DEFAULT_BABEL_ENV_OPTIONS);
    }
