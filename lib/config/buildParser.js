 ...
 * @returns {Array}
 */
function parseBuild (build, fileCache, fileExtensions, fileFactory, npmModulepaths, runtimeOptions, server, parent, level) {
  if (!parent) {
    numBuildTargets = 0;
    level = 0;
  }
  ...
  return build.reduce((build, buildTarget) => {
    ...
    if (!buildTarget.hasParent) {
      ...
      let fileFactoryOptions = { fileCache, fileExtensions, globalAliases, npmModulepaths, pluginOptions, runtimeOptions };
    ...
    if (hasInput) {
      ...
      if (buildTarget.build) {
        ...
        buildTarget.build = parseBuild(buildTarget.build, fileCache, fileExtensions, fileFactory, npmModulepaths, runtimeOptions, server, buildTarget, level + 1);
  ...
  if (buildTarget.input) {
    ...
    buildTarget.inputpaths = inputs.reduce((inputs, input) => {
      ...
      if (!allFileExtensions.includes(extension)) {
        warn(`${strong(extension)} is an unknown file type. You should install a language pack from npm to enable support. Search npm for "buddy-plugin" to see all available.`);
        include = false;
      }
