...
function getBuildFactory (buildConfig) {
  return function createBuild (inputpath) {
    const input = path.relative(process.cwd(), inputpath);
    const outputpath = path.dirname(buildConfig.outputpaths[0]);
    const config = Object.assign({}, buildConfig, {
      ...
      isAppServer: false,
      label: '',
      level: buildConfig.level + 1,
      output: buildConfig.output,
      outputpaths: [outputpath]
    });

    if (!buildConfig.builds.some((build) => build.input == input)) {
      buildConfig.builds.push(buildFactory(config));
      buildConfig.childInputpaths.push(inputpath);
    }
  };
}
...
function parseChildInputpaths (buildConfig) {
  function parse (build) {
    ...
    build.forEach((buildConfig) => {
      inputpaths = inputpaths.concat(buildConfig.inputpaths, buildConfig.builds ? parse(buildConfig.builds) : []);
    });

    return inputpaths;
  }

  buildConfig.childInputpaths = parse(buildConfig.builds);
}
...
function isAppServer (inputpaths, server) {
  ...
  return server != undefined
    ...
    && contains(inputpaths, server.file);
}