 ...
 * @returns {Array}
 */
function parseBuild (build, caches, fileExtensions, fileFactory, npmModulepaths, runtimeOptions, server, parent, level) {
  if (!parent) {
    numBuildTargets = 0;
    level = 0;
  }
  ...
  return build.reduce((build, buildTarget) => {
    ...
    if (hasInput) {
      ...
      if (buildTarget.build) {
        ...
        buildTarget.build = parseBuild(buildTarget.build, caches, fileExtensions, fileFactory, npmModulepaths, runtimeOptions, server, buildTarget, level + 1);
