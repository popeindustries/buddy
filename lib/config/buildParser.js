...
function parseBuild (builds, fileExtensions, fileFactory, npmModulepaths, runtimeOptions, serverConfig, parent, level) {
  ...
  return builds.reduce((builds, buildConfig) => {
    ...
    if (childBuilds) {
      build.builds = parseBuild(childBuilds, fileExtensions, fileFactory, npmModulepaths, runtimeOptions, serverConfig, build, level + 1);
      ...
      buildConfig.inputpaths = inputs.reduce((inputs, input) => {
        ...
        if (!path.extname(input).length) {
          ...
          inputs = inputs.concat(readdir(input, (resource, stat) => {
            ...
            if (isFile) inputsRelative.push(path.relative(hasTrailingSlash ? input : path.resolve(input, '..'), resource));
            return isFile;
          }));
        } else {
          inputs.push(input);
    ...
    if (outputs) {
      ...
      buildConfig.outputpaths = buildConfig.inputpaths.map((inputpath, idx) => {
        ...
        if (outputpath === inputpath) {
          throw Error(`the input ${strong(buildConfig.input)} is configured to be overwritten by ${strong(buildConfig.output)}. If you're trying to build the contents of a directory, remember to add a trailing slash to ${strong(buildConfig.input)}`);
        }
