...
const { getEnvironmentFromVersion, loadBuildPlugins } = require('./pluginLoader');
...
function parseBuild (build, caches, fileExtensions, fileFactory, npmModulepaths, runtimeOptions, server, parent) {
  ...
  return build.reduce((build, buildTarget) => {
    ...
    if (!buildTarget.hasParent) {
      ...
      buildTarget.browser = getEnvironmentFromVersion(buildTarget.version);

      // Generate plugin options based on 'options' and target 'version'
      const pluginOptions = buildTarget.options = loadBuildPlugins(buildTarget.options, buildTarget.version);
