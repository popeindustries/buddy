 ...
 * @returns {Array}
 */
function parseBuild (build, caches, fileExtensions, fileFactory, npmModulepaths, runtimeOptions, server, sources, parent) {
  ...
  return build.reduce((build, buildTarget) => {
    ...
    if ('input' in buildTarget && buildTarget.input != null) {
      ...
      if (buildTarget.build) {
        ...
        buildTarget.build = parseBuild(buildTarget.build, caches, fileExtensions, fileFactory, npmModulepaths, runtimeOptions, server, sources, buildTarget);
