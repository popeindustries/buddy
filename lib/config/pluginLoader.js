  ...
  loadBuildPlugins (options, version = []) {
    ...
    const missingDependenciesString = Object.keys(options)
      .reduce((dependencies, key) => {
        return dependencies.concat(extractDependencyStrings(options[key]));
      }, [])
      .filter((dependency) => {
        return resolve(path.resolve('package.json'), dependency) == '';
      })
      .join(' ');
  ...
  function init (items) {
    ...
    if (Array.isArray(items)) {
      items.forEach((item, idx, items) => {
        ...
        if ('string' == typeof id && (filepath = resolve(path.resolve('package.json'), id))) {
          if (isArray) {
