...
module.exports = {
  ...
  loadBuildPlugins (options = {}, version = []) {
    ...
    for (const type in options) {
      loadBuildPluginsForType(type, options[type], version);
    }
  ...
  /**
   * Retrieve target environment (node or browser) from 'version'
   * @param {Array} version
   * @returns {Boolean}
   */
  getEnvironmentFromVersion (version) {
    if (!Array.isArray(version)) version = [version];
 ...
 * @param {Array} version
 */
function loadBuildPluginsForType (type, options, version) {
  ...
  plugins = version.reduce((plugins, preset) => {
    ...
    const presetPlugins = allPlugins[type][preset];

    // Ignore generic or missing
    if (RE_GENERIC_VERSION.test(preset) || !presetPlugins) return plugins;
    return plugins.concat(presetPlugins);
  }, plugins);
  ...
  const missingDependenciesString = extractDependencyStrings(options)
  ...
  resolveDependecyStrings(options);
}

/**
 * Extract dependency strings from 'options'
 * @param {Object} options
 * @returns {Array}
 */
function extractDependencyStrings (options) {
  let dependencies = [];
...
/**
 * Resolve dependency strings in 'options' to modules
 * @param {Object} options
 */
function resolveDependecyStrings (options) {
  function init (items) {
    // Invalid if not Array
    if (Array.isArray(items)) {
