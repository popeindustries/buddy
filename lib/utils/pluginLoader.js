...
module.exports = {
  ...
  loadPluginModules (config) {
    ...
    const defaultPluginModules = path.resolve(__dirname, '../plugins');
    ...
    loadPluginModulesFromDir(defaultPluginModules, config);
  ...
  loadBuildPlugins (options, version = []) {
    ...
    let plugins = version.reduce((plugins, preset) => {
      if (!babel[preset.toLowerCase()]) {
        warn(`${strong(preset)} is not a recognised build target version. Additional versions should be installed with npm`);
        return plugins;
      }
      return plugins.concat(babel[preset.toLowerCase()]);
    }, babel.default.slice());
