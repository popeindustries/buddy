...
const { ReloadServer, Server } = require('buddy-server');
...
module.exports = {
    ...
    if (serve) {
      tasks.push((done) => {
        ...
        const started = (err) => {
          ...
          print(`started serving ${strong(options.file || options.directory)} on port: ${options.port}`, 1);
          done();
        };
  ...
  restart (fn) {
    if (isReady && appServer) {
      ...
      server.kill();
      setTimeout(callable(module, 'startAppServer', fn), 100);
    }
  },
