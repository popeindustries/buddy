...
module.exports = {
   ...
   * @param {Object} config
   */
  loadCorePlugins (config) {
    const cwd = process.cwd();
    const defaultModules = path.resolve('../plugins');
    ...
    loadBuddyPlugins(defaultModules, config);
    // Load from project dir
    loadBuddyPlugins(projectModules, config);
    // Load from buddy dir if buddy not installed in project dir
    if (!~buddyModules.indexOf(cwd)) loadBuddyPlugins(buddyModules, config);
  },
 ...
 * @param {Object} config
 */
function loadBuddyPlugins (dir, config) {
  if (!fs.existsSync(dir)) return;

  fs.readdirSync(dir)
    .filter((resource) => {
      return RE_PLUGIN.test(resource)
        || ~dir.indexOf('plugins') && RE_JS_FILE.test(resource);
    })
    .forEach((resource) => {
