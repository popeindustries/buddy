'use strict';

const { SourceMapConsumer } = require('source-map');
const isEqual = require('lodash/isEqual');
const unique = require('lodash/uniqWith');

const COLUMN = ' ';
// Line starting with '//'
const RE_COMMENT_SINGLE_LINE = /^\s*(?:\/\/|#).+$/gm;
// Multi line block '/** ... */'
const RE_COMMENT_MULTI_LINES = /((?:\/\*(?:[^*]|(?:\*+[^*\/]))*\*+\/))$/gm;
const RE_LINE_BEGIN = /^/gm;
const SEG_LENGTH = 30;

module.exports = {
  commentStrip,
  commentWrap,
  indent,
  uniqueMatch,
  regexpEscape,
  truncate,
  replace,
  getLocationFromIndex
};

/**
 * Strip comments from 'string'
 * @param {String} string
 * @returns {String}
 */
function commentStrip (string) {
  // Remove commented lines
  string = string.replace(RE_COMMENT_SINGLE_LINE, '');
  string = string.replace(RE_COMMENT_MULTI_LINES, '');
  return string;
}

/**
 * Wrap 'string' in comment based on 'type'
 * @param {String} string
 * @param {String} type
 * @returns {String}
 */
function commentWrap (string, type) {
  let open, close;

  if (type == 'html') {
    open = '<!-- ';
    close = ' -->';
  } else {
    open = '/* ';
    close = ' */';
  }

  return open + string + close;
}

/**
 * Indent the given 'string' a specific number of columns
 * @param {String} string
 * @param {Int} column
 * @returns {String}
 */
function indent (string, column) {
  const spaces = (new Array(++column)).join(COLUMN);

  return string.replace(RE_LINE_BEGIN, spaces);
}

/**
 * Match unique occurrences in 'string'
 * @param {String} string
 * @param {RegExp} regexp
 * @returns {Array}
 */
function uniqueMatch (string, regexp) {
  let results = [];
  let match;

  while (match = regexp.exec(string)) {
    results.push({
      context: match[0],
      match: match[1] || ''
    });
  }

  // Filter duplicates
  return unique(results, isEqual);
}

/**
 * Escape 'string' for use in RegExp constructor
 * @param {String} string
 * @returns {String}
 */
function regexpEscape (string) {
  return string.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
}

/**
 * Truncate 'string'
 * @param {String} string
 * @returns {String}
 */
function truncate (string) {
  if (string.length > (SEG_LENGTH * 2) + 3) {
    return string.slice(0, SEG_LENGTH) + '...' + string.slice(-SEG_LENGTH);
  }

  return string;
}

/**
 * Replace 'search' in 'string' with 'replacement',
 * updating 'map' with changes
 * @param {String} string
 * @param {String} search
 * @param {String} replacement
 * @param {SourceMapGenerator} map
 * @param {String} filepath
 * @returns {String}
 */
function replace (string, search, replacement, map, filepath) {
  // const lines = string.split('\n');
  // const mapConsumer = new SourceMapConsumer(map);
  // const replacementLength = replacement.length;
  // const searchLength = search.length;
  // let index = 0;
  // let replacements = [];

  // for (let ln = 0, n = lines.length; ln < n; ln++) {
  //   let line = lines[ln];
  //   let last = 0;
  //   let col;

  //   while ((col = line.indexOf(search, last)) != -1) {
  //     string = string.substring(0, index + col)
  //       + replacement
  //       + string.substring(index + col + searchLength);
  //     line = line.substring(0, col)
  //       + replacement
  //       + line.substring(col + searchLength);
  //     replacements.push({ line: ln, column: col });
  //     last = col + replacementLength;
  //   }

  //   // Add removed line ending
  //   index += line.length + 1;
  // }

  return string;
}

/**
 * Retrieve line/column from 'index' of 'string'
 * Column is zero-indexed
 * @param {String} string
 * @param {Number} index
 * @returns {Object}
 */
function getLocationFromIndex (string, index) {
  const lines = string.split('\n');
  const idx = 0;

  for (let i = 0, n = lines.length; i < n; i++) {
    const line = lines[i];
    // Add removed line ending
    const lineLength = line.length + 1;

    if (idx + lineLength > index) {
      return {
        line: i + 1,
        column: index - idx
      };
    }

    idx += lineLength;
  }

  return {
    line: 0,
    column: 0
  };
}