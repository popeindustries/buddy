 ...
 * @returns {Array}
 */
function parseBuild (build, caches, fileExtensions, fileFactory, runtimeOptions, server, parent) {
  ...
  return build.reduce((build, buildTarget) => {
    const sources = ['.'].concat(buildTarget.sources || []).map((source) => path.resolve(source));
    ...
    if (!buildTarget.hasParent) {
      let fileFactoryOptions = { caches, fileExtensions, runtimeOptions, sources };
      ...
      pluginLoader.loadBuildPlugins(buildTarget.options, buildTarget.version);
    } else {
    ...
    if ('input' in buildTarget && buildTarget.input != null) {
      ...
      if (buildTarget.build) {
        ...
        buildTarget.build = parseBuild(buildTarget.build, caches, fileExtensions, fileFactory, runtimeOptions, server, buildTarget);
