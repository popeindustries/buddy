...
const { readdir: { sync: readdir } } = require('recur-fs');
...
module.exports = {
  ...
  parse (config) {
    ...
    config.build = parseBuild(config.build, config.fileExtensions, config.fileExtensionsByType, config.runtimeOptions);
  }
};
 ...
 * @returns {Array}
 */
function parseBuild (build, fileExtensions, fileExtensionsByType, runtimeOptions, parent) {
  ...
  return build.reduce((build, buildTarget) => {
    ...
    if (!('input' in buildTarget) || buildTarget.input != null) {
      ...
      if (buildTarget.build) {
        ...
        buildTarget.build = parseBuild(buildTarget.build, fileExtensions, fileExtensionsByType, runtimeOptions, buildTarget);
...
function parseInputOutput (buildTarget, fileExtensions, fileExtensionsByType, runtimeOptions) {
  ...
  if (buildTarget.input) {
    ...
    buildTarget.inputpaths = inputs.reduce((inputs, input) => {
      ...
      if (!~fileExtensions.indexOf(input)) {
        include = false;
      }
