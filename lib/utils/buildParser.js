...
const { strong, warn } = require('./cnsl');
...
module.exports = {
  ...
  parse (config) {
    ...
    config.build = parseBuild(config.build, config.fileExtensions, config.fileExtensionsByType, config.runtimeOptions, config.server);
  }
};
 ...
 * @param {Object} fileExtensionsByType
 * @param {Object} runtimeOptions
 * @param {Object} server
 * @param {Object} [parent]
 * @returns {Array}
 */
function parseBuild (build, fileExtensions, fileExtensionsByType, runtimeOptions, server, parent) {
  ...
  return build.reduce((build, buildTarget) => {
    ...
    if ('js' in buildTarget || 'css' in buildTarget || 'html' in buildTarget || 'targets' in buildTarget) {
      warn('this build format is no longer compatible with newer versions of Buddy. See https://github.com/popeindustries/buddy/blob/master/docs/config.md for help');
      return build;
    }
    // Deprecate aliases
    ...
    parseInputOutput(buildTarget, fileExtensions, fileExtensionsByType, runtimeOptions);
    ...
    if ('input' in buildTarget && buildTarget.input != null) {
      ...
      if (buildTarget.build) {
        ...
        buildTarget.build = parseBuild(buildTarget.build, fileExtensions, fileExtensionsByType, runtimeOptions, server, buildTarget);
 ...
 * @param {RegExp} fileExtensions
 * @param {Object} fileExtensionsByType
 * @param {Object} runtimeOptions
 */
function parseInputOutput (buildTarget, fileExtensions, fileExtensionsByType, runtimeOptions) {
  ...
  if (buildTarget.input) {
    ...
    buildTarget.inputpaths = inputs.reduce((inputs, input) => {
      ...
      if (!~fileExtensions.indexOf(extension)) {
        include = false;
      }
    ...
    if (outputs) {
      ...
      buildTarget.outputpaths = buildTarget.inputpaths.map((inputpath, idx) => {
        ...
        const type = filetype(inputpath, fileExtensionsByType);
