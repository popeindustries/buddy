...
class Buddy {
  ...
  build (fn) {
    ...
    this.run(this.builds, (err, filepaths) => {
      ...
      print(`done building in ${chalk.cyan((stop('build') / 1000) + 's')}`, 1);
      // Run script
      this.executeScript();
  ...
  initBuilds (builds, runtimeOptions) {
    const init = (builds, isChild) => {
      return builds.map((build) => {
        ...
        // Setup watch
        if (!isChild && runtimeOptions.watch) {
          build.fileCache.on('change', this.onFileCacheChange);
  ...
  onFileCacheChange (file) {
    ...
    if (!this.building) {
      ...
      this.run(builds, (err, filepaths) => {
        ...
        print(`done re-building in ${chalk.cyan((stop('watch') / 1000) + 's')}`, 1);
        // Run test script
        this.executeScript();
      });
    }
