...
const { deriveType } = require('./utils');
...
function getDetails (filepath, options) {
  ...
  const paths = [...options.sources, ...resolveNodeModules(pkgpath), ...options.globalSources];
  const type = deriveType(filepath, options.fileExtensions);
    ...
    if (json) {
      ...
      if (json.browser) {
        if ('string' == typeof json.browser) {
          ...
          details.aliases = Object.assign(details.aliases, alias.parse(manifestRoot, json.browser, type, options.fileExtensions));
...
function resolvePath (filepath) {
  ...
  const cwd = process.cwd();
  const npmPackage = filepath.includes(path.resolve(cwd, 'node_modules'));
...
function resolveId (details, filepath) {
  ...
  if ('string' == typeof filepath) {
    ...
    const version = details.id.indexOf(VERSION_DELIMITER)
      ? VERSION_DELIMITER + details.version
      : '';

    if (filepath == details.main) return details.id + version;
