...
const { deriveType, findFile, isFilepath } = require('./utils');
...
function getDetails (filepath, options) {
  ...
  const { fileExtensions, sources } = options;
  ...
  const type = deriveType(filepath, fileExtensions) || 'js';
  ...
  if (fs.existsSync(manifestpath)) {
    ...
    if (json) {
      ...
      details.main = findFile(path.join(manifestRoot, json.main || 'index.js'), type, fileExtensions);
