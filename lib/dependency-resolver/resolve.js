...
const { deriveType, isAbsoluteFilepath, isFilepath, isRelativeFilepath, findFile } = require('./utils');
...
function find (id, type, sourcedir, options) {
  ...
  let filepath = alias.resolve(id, pkgDetails && pkgDetails.aliases);

  if (filepath === false || options.nativeModules.includes(filepath)) return false;
  if (!isFilepath(filepath) && sourcedir.includes('node_modules')) {
  ...
  console.log(id, filepath, sourcedir)
  console.log(pkgDetails)

  if (isAbsoluteFilepath(filepath)) {
    filepath = findFile(filepath, type, options.fileExtensions);
    filepath = alias.resolve(filepath, pkgDetails && pkgDetails.aliases);
    ...
    // Only update id if has been resolved as package
