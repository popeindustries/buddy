...
const { findFile, isFilepath, isRelativeFilepath } = require('./utils');
  ...
  parse (pkgpath, aliases, type, fileExtensions) {
    ...
    for (let key in aliases) {
      ...
      if (type && fileExtensions) {
        if (isFilepath(key)) key = findFile(key, type, fileExtensions) || key;
        if (isFilepath(value)) value = findFile(value, type, fileExtensions) || value;
      }
